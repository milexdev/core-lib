...
class CampaignController extends AbstractStandardFormController
{
    ...
    public function contactsAction($objectId, $page = 1, $count = null, \DateTimeInterface $dateFrom = null, \DateTimeInterface $dateTo = null)
    {
        ...
        $session->set('milex.campaign.contact.page', $page);
    ...
    public function indexAction($page = null)
    {
        ...
        $session = $this->get('session');
        if (empty($page)) {
            $page = $session->get('milex.campaign.page', 1);
        }

        //set limits
        $limit = $session->get('milex.campaign.limit', $this->coreParametersHelper->get('default_pagelimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('milex.campaign.filter', ''));
        $session->set('milex.campaign.filter', $search);
        ...
        $orderBy    = $session->get('milex.campaign.orderby', 'c.dateModified');
        $orderByDir = $session->get('milex.campaign.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('milex.campaign.page', $lastPage);
            $returnUrl = $this->generateUrl('milex_campaign_index', ['page' => $lastPage]);
        ...
        $session->set('milex.campaign.page', $page);

        $viewParameters = [
            'permissionBase'  => $this->getPermissionBase(),
            'milexContent'   => $this->getJsLoadMethodPrefix(),
    ...
    public function newAction()
    {
        ...
        if (!$this->get('milex.security')->isGranted('campaign:campaigns:create')) {
            return $this->accessDenied();
        }

        //set the page we came from
        $page = $this->get('session')->get('milex.campaign.page', 1);

        $options = $this->getEntityFormOptions();
        $action  = $this->generateUrl('milex_campaign_action', ['objectAction' => 'new']);
                ...
                if ($valid = $this->isFormValid($form)) {
                    if ($valid = $this->beforeEntitySave($campaign, $form, 'new')) {
                        ...
                        if (method_exists($this, 'viewAction')) {
                            ...
                            $returnUrl      = $this->generateUrl('milex_campaign_action', $viewParameters);
                            $template       = 'MauticCampaignBundle:Campaign:view';
                        } else {
                            $viewParameters = ['page' => $page];
                            $returnUrl      = $this->generateUrl('milex_campaign_index', $viewParameters);
        ...
        $delegateArgs = [
            'viewParameters' => [
                ...
                'milexContent'   => 'campaign',
                'actionRoute'     => 'milex_campaign_action',
                'indexRoute'      => 'milex_campaign_index',
                'tablePrefix'     => 'c',
                'modelName'       => 'campaign',
                'translationBase' => $this->getTranslationBase(),
    ...
    {
        $tempId   = 'milex_'.sha1(uniqid(mt_rand(), true));
    ...
    protected function beforeEntitySave($entity, Form $form, $action, $objectId = null, $isClone = false)
    {
        if (empty($this->campaignEvents)) {
            ...
            $form->addError(
                new FormError(
                    $this->get('translator')->trans('milex.campaign.form.events.notempty', [], 'validators')
                )
            );
        ...
        if (empty($this->campaignSources['lists']) && empty($this->campaignSources['forms'])) {
            ...
            $form->addError(
                new FormError(
                    $this->get('translator')->trans('milex.campaign.form.sources.notempty', [], 'validators')
                )
            );
    ...
    {
        $session = $this->get('session');
        $session->remove('milex.campaign.'.$id.'.events.modified');
        $session->remove('milex.campaign.'.$id.'.events.deleted');
        $session->remove('milex.campaign.'.$id.'.events.canvassettings');
        $session->remove('milex.campaign.'.$id.'.leadsources.current');
        $session->remove('milex.campaign.'.$id.'.leadsources.modified');
        $session->remove('milex.campaign.'.$id.'.leadsources.deleted');
    }
    ...
    protected function getCampaignSessionId(Campaign $campaign, $action, $objectId = null)
    {
        ...
        if ($objectId) {
            ...
            $sessionId = 'milex_'.sha1(uniqid(mt_rand(), true));
    ...
    {
        $session        = $this->get('session');
        $currentFilters = $session->get('milex.campaign.list_filters', []);
        $updatedFilters = $this->request->get('filters', false);

        $sourceLists = $this->getCampaignModel()->getSourceLists();
        $listFilters = [
            'filters' => [
                'placeholder' => $this->get('translator')->trans('milex.campaign.filter.placeholder'),
                'multiple'    => true,
                'groups'      => [
                    'milex.campaign.leadsource.form' => [
                        'options' => $sourceLists['forms'],
                        'prefix'  => 'form',
                    ],
                    'milex.campaign.leadsource.list' => [
        ...
        $session->set('milex.campaign.list_filters', $currentFilters);

        $joinLists = $joinForms = false;
        if (!empty($currentFilters)) {
            $listIds = $catIds = [];
            foreach ($currentFilters as $type => $typeFilters) {
                $listFilters['filters']['groups']['milex.campaign.leadsource.'.$type]['values'] = $typeFilters;
    ...
    protected function getSessionEvents($id)
    {
        ...
        $modifiedEvents = $session->get('milex.campaign.'.$id.'.events.modified', []);
        $deletedEvents  = $session->get('milex.campaign.'.$id.'.events.deleted', []);
    ...
    protected function getSessionSources($id, $isClone = false)
    {
        ...
        $campaignSources = $session->get('milex.campaign.'.$id.'.leadsources.current', []);
        $modifiedSources = $session->get('milex.campaign.'.$id.'.leadsources.modified', []);
    ...
    protected function getViewArguments(array $args, $action): array
    {
        ...
        $eventCollector = $this->get('milex.campaign.event_collector');
        ...
        switch ($action) {
            ...
            case 'view':
                ...
                $action          = $this->generateUrl('milex_campaign_action', ['objectAction' => 'view', 'objectId' => $objectId]);
                $dateRangeForm   = $this->get('form.factory')->create(DateRangeType::class, $dateRangeValues, ['action' => $action]);
                $events          = $this->getCampaignModel()->getEventRepository()->getCampaignEvents($entity->getId());
                $dateFrom        = null;
                $dateTo          = null;
                $dateToPlusOne   = null;
                $this->setCoreParametersHelper($this->get('milex.config'));
    ...
    protected function prepareCampaignEventsForEdit($entity, $objectId, $isClone = false)
    {
        ...
        $dateHelper     = $this->get('milex.helper.template.date');
            ...
            switch ($event['triggerMode']) {
                case 'interval':
                    $label = $translator->trans(
                        'milex.campaign.connection.trigger.interval.label'.('no' == $event['decisionPath'] ? '_inaction' : ''),
                        [
                            '%number%' => $event['triggerInterval'],
                            '%unit%'   => $translator->trans(
                                'milex.campaign.event.intervalunit.'.$event['triggerIntervalUnit'],
                ...
                case 'date':
                    $label = $translator->trans(
                        'milex.campaign.connection.trigger.date.label'.('no' == $event['decisionPath'] ? '_inaction' : ''),
                        [
        ...
        $this->get('session')->set('milex.campaign.'.$objectId.'.events.modified', $campaignEvents);
    }
    ...
    protected function prepareCampaignSourcesForEdit($objectId, $campaignSources, $isPost = false)
    {
        ...
        if (!$isPost) {
            $session = $this->get('session');
            $session->set('milex.campaign.'.$objectId.'.leadsources.current', $campaignSources);
            $session->set('milex.campaign.'.$objectId.'.leadsources.modified', $campaignSources);
        }
    }
    ...
    {
        $this->get('session')->set('milex.campaign.'.$sessionId.'.events.canvassettings', $canvasSettings);
    }
    ...
    {
        return $this->get('session')->get('milex.campaign.'.$sessionId.'.events.canvassettings');
    }
