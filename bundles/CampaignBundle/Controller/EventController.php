...
class EventController extends CommonFormController
{
    ...
    public function newAction()
    {
        ...
        if (!$type
            ...
            || !$this->get('milex.security')->isGranted(
                [
        ...
        $form   = $this->get('form.factory')->create(
            ...
            $event,
            [
                'action'   => $this->generateUrl('milex_campaignevent_action', ['objectAction' => 'new']),
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $modifiedEvents = $session->get('milex.campaign.'.$campaignId.'.events.modified');
                    ...
                    $session->set('milex.campaign.'.$campaignId.'.events.modified', $modifiedEvents);
                } else {
                    $success = 0;
                }
            }
        ...
        if (!empty($keyId)) {
            ...
            if ('interval' == $event['triggerMode']) {
                $label = 'milex.campaign.connection.trigger.interval.label';
                if ('no' == $anchorName) {
                    $label .= '_inaction';
                }
                ...
                $label = 'milex.campaign.connection.trigger.date.label';
                if ('no' == $anchorName) {
                    $label .= '_inaction';
                }
                $passthroughVars['label'] = $translator->trans(
    ...
    public function editAction($objectId)
    {
        ...
        $form            = $this->get('form.factory')->create(
            ...
            $event,
            [
                'action'   => $this->generateUrl('milex_campaignevent_action', ['objectAction' => 'edit', 'objectId' => $objectId]),
        ...
        if (!$cancelled && $valid) {
            ...
            if (Event::TRIGGER_MODE_DATE === $event['triggerMode']) {
                ...
                /** @var \Mautic\CoreBundle\Templating\Helper\DateHelper $dh */
                $dh                       = $this->get('milex.helper.template.date');
                $passthroughVars['label'] = $this->translator->trans(
                    $label,
                    [
                        '%full%' => $dh->toFull($event['triggerDate']),
    ...
    public function deleteAction($objectId)
    {
        ...
        $session        = $this->get('session');
        $modifiedEvents = $session->get('milex.campaign.'.$campaignId.'.events.modified', []);
        $deletedEvents  = $session->get('milex.campaign.'.$campaignId.'.events.deleted', []);
        ...
        if ('POST' == $this->request->getMethod() && null !== $event) {
            ...
            if (!in_array($objectId, $deletedEvents)) {
                ...
                if (false === strpos($objectId, 'new')) {
                    ...
                    $session->set('milex.campaign.'.$campaignId.'.events.deleted', $deletedEvents);
                }
