...
class SourceController extends CommonFormController
{
    ...
    public function newAction($objectId = 0)
    {
        ...
        $form       = $this->get('form.factory')->create(
            ...
            $source,
            [
                'action'         => $this->generateUrl('milex_campaignsource_action', ['objectAction' => 'new', 'objectId' => $objectId]),
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $modifiedSources              = $session->get('milex.campaign.'.$objectId.'.leadsources.modified');
                    $modifiedSources[$sourceType] = array_flip($form[$sourceType]->getData());
                    $session->set('milex.campaign.'.$objectId.'.leadsources.modified', $modifiedSources);
                } else {
                    $success = 0;
                }
            }
    ...
    public function editAction($objectId)
    {
        $session         = $this->get('session');
        $method          = $this->request->getMethod();
        $modifiedSources = $selectedSources = $session->get('milex.campaign.'.$objectId.'.leadsources.modified', []);
        if ('POST' == $method) {
            $source     = $this->request->request->get('campaign_leadsource');
            $sourceType = $source['sourceType'];
        } else {
        ...
        $form       = $this->get('form.factory')->create(
            ...
            $source,
            [
                'action'         => $this->generateUrl('milex_campaignsource_action', ['objectAction' => 'edit', 'objectId' => $objectId]),
                'source_choices' => $sourceList,
            ]
        );
