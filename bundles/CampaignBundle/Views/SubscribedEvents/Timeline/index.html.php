...
if (!empty($item['metadata']['errors'])) {
    ...
    $errors = (!empty($item['metadata']['reason'])) ? $item['metadata']['reason'] : 'milex.campaign.event.failed.timeline';
...
<?php if ($item['isScheduled']): ?>
    ...
    echo ' hide';
                ...
                <?php echo $view['translator']->trans('milex.core.timeline.event.scheduled.time', ['%date%' => $dateSpan, '%event%' => $event['eventLabel']]); ?>
            </span>
            <span class="timeline-campaign-event-cancelled-<?php echo $item['event_id']; ?><?php if (!$cancelled) {
    echo ' hide';
} ?>">
                <?php echo $view['translator']->trans('milex.campaign.event.cancelled.time', ['%date%' => $dateSpan, '%event%' => $event['eventLabel']]); ?>
            </span>
        </span>
        ...
        <span class="form-buttons btn-group btn-group-xs mb-3" role="group" aria-label="Field options">
            <button type="button" id="timeline-campaign-event-save-<?php echo $item['event_id']; ?>" class="btn btn-default btn-nospin" onmousedown="return false;" onclick="Mautic.saveScheduledCampaignEvent(<?php echo $item['event_id']; ?>, <?php echo $lead->getId(); ?>)" data-toggle="tooltip" title="<?php echo $view['translator']->trans('milex.campaign.event.save'); ?>" style="display:none">
                <i class="fa fa-floppy-o text-primary"></i>
            </button>
            <button type="button" class="btn btn-default btn-nospin btn-reschedule" onclick="Mautic.updateScheduledCampaignEvent(<?php echo $item['event_id']; ?>, <?php echo $lead->getId(); ?>)" data-toggle="tooltip" title="<?php echo $view['translator']->trans('milex.campaign.event.reschedule'); ?>">
                <i class="fa fa-clock-o text-primary"></i>
            </button>
            <button type="button" class="btn btn-default btn-nospin"<?php if ($cancelled) {
    echo ' disabled';
} ?> onclick="Mautic.cancelScheduledCampaignEvent(<?php echo $item['event_id']; ?>, <?php echo $lead->getId(); ?>)" data-toggle="tooltip" title="<?php echo $view['translator']->trans('milex.campaign.event.cancel'); ?>">
...
<?php if ($errors): ?>
    ...
    <hr />
    <?php endif; ?>
    <p class="text-danger mt-0 mb-10"><i class="fa fa-warning"></i> <?php echo $view['translator']->trans('milex.campaign.event.last_error').': '.$errors; ?></p>
<?php endif; ?>
...
<?php if (!empty($item['metadata']['timeline']) || $item['campaign_description'] || $item['event_description']): ?>
    ...
    <?php if ($item['campaign_description']): ?>
        <p class="mt-0 mb-10"><?php echo $view['translator']->trans('milex.campaign.campaign.description', ['%description%' => $item['campaign_description']]); ?></p>
    <?php endif; ?>
    <?php if ($item['event_description']): ?>
        <p class="mt-0 mb-10"><?php echo $view['translator']->trans('milex.campaign.campaign.description', ['%description%' => $item['event_description']]); ?></p>
    <?php endif; ?>
<?php endif; ?>
</div>
