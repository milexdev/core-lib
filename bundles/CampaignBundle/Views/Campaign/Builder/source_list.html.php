...
<div id="SourceGroupList" class="hide">
    <h4 class="mb-xs">
        <span><?php echo $view['translator']->trans('milex.campaign.leadsource.header'); ?></span>
    </h4>
    <select id="SourceList" class="campaign-event-selector">
        ...
        <?php foreach (['lists', 'forms'] as $option): ?>
            ...
            <option id="campaignLeadSource_<?php echo $option; ?>"
                    ...
                    title="<?php echo $view->escape($view['translator']->trans('milex.campaign.leadsource.'.$option.'.tooltip')); ?>"
                    value="<?php echo $view->escape($option); ?>"
                <?php if (!empty($campaignSources[$option])) {
                        echo 'disabled';
                    } ?>>
                <span><?php echo $view['translator']->trans('milex.campaign.leadsource.'.$option); ?></span>
            </option>
        <?php endforeach; ?>
    </select>
</div>
