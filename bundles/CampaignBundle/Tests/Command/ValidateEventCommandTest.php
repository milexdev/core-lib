...
class ValidateEventCommandTest extends AbstractCampaignCommand
{
    public function testEventsAreExecutedForInactiveEventWithSingleContact()
    {
        ...
        $this->runCommand('milex:campaigns:validate', ['--decision-id' => 3, '--contact-id' => 1]);
    ...
    {
        $this->runCommand('milex:campaigns:trigger', ['-i' => 1, '--contact-ids' => '1,2,3']);
        ...
        // Now they should be inactive
        $this->runCommand('milex:campaigns:validate', ['--decision-id' => 3, '--contact-ids' => '1,2,3']);
