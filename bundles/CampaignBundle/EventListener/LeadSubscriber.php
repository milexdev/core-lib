...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    {
        $this->addTimelineEvents($event, 'campaign.event', $this->translator->trans('milex.campaign.triggered'));
        $this->addTimelineEvents($event, 'campaign.event.scheduled', $this->translator->trans('milex.campaign.scheduled'));
    }
        ...
        if (!$event->isEngagementCount()) {
            foreach ($logs['results'] as $log) {
                ...
                if (empty($log['isScheduled']) && empty($log['dateTriggered'])) {
                    // Note as cancelled
                    $label .= ' <i data-toggle="tooltip" title="'.$this->translator->trans('milex.campaign.event.cancelled')
                ...
                if ((!empty($log['metadata']['errors']) && empty($log['dateTriggered'])) || !empty($log['metadata']['failed']) || !empty($log['fail_reason'])) {
                    $label .= ' <i data-toggle="tooltip" title="'.$this->translator->trans('milex.campaign.event.has_last_attempt_error')
