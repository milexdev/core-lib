...
class TriggerCampaignCommand extends ModeratedCommand
{
    ...
    {
        $this
            ->setName('milex:campaigns:trigger')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($id) {
            ...
            if ($campaign = $this->campaignRepository->getEntity($id)) {
                ...
                $output->writeln('<error>'.$this->translator->trans('milex.campaign.rebuild.not_found', ['%id%' => $id]).'</error>');
            }
    ...
    private function triggerCampaign(Campaign $campaign)
    {
        ...
        try {
            $this->output->writeln('<info>'.$this->translator->trans('milex.campaign.trigger.triggering', ['%id%' => $campaign->getId()]).'</info>');
            // Reset batch limiter
    ...
    private function executeKickoff()
    {
        ...
        $this->output->writeln('<comment>'.$this->translator->trans('milex.campaign.trigger.starting').'</comment>');
    ...
    {
        $this->output->writeln('<comment>'.$this->translator->trans('milex.campaign.trigger.scheduled').'</comment>');
    ...
    private function executeInactive()
    {
        ...
        $this->output->writeln('<comment>'.$this->translator->trans('milex.campaign.trigger.negative').'</comment>');
