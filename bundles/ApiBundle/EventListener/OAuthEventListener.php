...
class OAuthEventListener
{
    ...
    public function __construct(EntityManager $entityManager, CorePermissions $corePermissions, TranslatorInterface $translator)
    {
        ...
        $this->milexSecurity = $corePermissions;
    ...
    public function onPreAuthorizationProcess(OAuthEvent $event)
    {
        if ($user = $this->getUser($event)) {
            ...
            if (!$this->milexSecurity->isGranted('api:access:full')) {
                throw new AccessDeniedException($this->translator->trans('milex.core.error.accessdenied', [], 'flashes'));
            }
