...
class CommonApiController extends AbstractFOSRestController implements MauticController
{
    ...
    public function deleteEntitiesAction()
    {
        ...
        foreach ($entities as $key => $entity) {
            if (null === $entity || !$entity->getId()) {
                $this->setBatchError($key, 'milex.core.error.notfound', Response::HTTP_NOT_FOUND, $errors, $entities, $entity);
                continue;
            }

            if (!$this->checkEntityAccess($entity, 'delete')) {
                $this->setBatchError($key, 'milex.core.error.accessdenied', Response::HTTP_FORBIDDEN, $errors, $entities, $entity);
                continue;
            }
    ...
    {
        $this->security = $this->get('milex.security');
     ...
     * @return Response
     */
    protected function accessDenied($msg = 'milex.core.error.accessdenied')
    {
     ...
     * @return Response
     */
    protected function badRequest($msg = 'milex.core.error.badrequest')
    {
    ...
    {
        return $this->get('milex.model.factory')->getModel($modelNameKey);
    }
     ...
     * @return Response
     */
    protected function notFound($msg = 'milex.core.error.notfound')
    {
    ...
    protected function prepareEntityResultsToArray($results, $callback = null)
    {
         ...
         */
        $entityResultHelper = $this->get('milex.api.helper.entity_result');
    ...
    protected function processForm($entity, $parameters = null, $method = 'PUT')
    {
        ...
        if ($form->isValid()) {
            ...
            if (in_array($statusCode, [Response::HTTP_CREATED, Response::HTTP_ACCEPTED])) {
                $route = (null !== $this->get('router')->getRouteCollection()->get('milex_api_'.$this->entityNameMulti.'_getone'))
                    ? 'milex_api_'.$this->entityNameMulti.'_getone' : 'milex_api_get'.$this->entityNameOne;
            ...
            if (!$msg) {
                $msg = $this->translator->trans('milex.core.error.badrequest', [], 'flashes');
            }
    ...
    {
        $batchLimit = (int) $this->get('milex.config')->getParameter('api_batch_max_limit', 200);
        if (count($parameters) > $batchLimit) {
            return $this->returnError($this->get('translator')->trans('milex.api.call.batch_exception', ['%limit%' => $batchLimit]));
        }
