...
class SysinfoModel
{
    ...
    public function getPhpInfo()
    {
        ...
        if (function_exists('phpinfo') && 'cli' !== php_sapi_name()) {
            ...
            $this->phpInfo = $this->translator->trans('milex.sysinfo.phpinfo.phpversion', ['%phpversion%' => phpversion()]);
        } else {
            $this->phpInfo = $this->translator->trans('milex.sysinfo.phpinfo.missing');
        }
    ...
    {
        $log = $this->coreParametersHelper->get('log_path').'/milex_'.MILEX_ENV.'-'.date('Y-m-d').'.php';
