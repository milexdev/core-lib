...
namespace Milex\IntegrationsBundle\Sync\DAO\Sync;
...
class InputOptionsDAO
{
     ...
     *      'milex-object-id' => ['contact:12', 'contact:13'] or a ObjectIdsDAO object,
    ...
    public function __construct(array $input)
    {
        ...
        $this->milexObjectIds      = $this->validateObjectIds($input, 'milex-object-id');
    ...
    public function getMilexObjectIds(): ?ObjectIdsDAO
    {
        return $this->milexObjectIds;
    }
    ...
    /**
     * This method exists only because Milex leads were renamed to contacts. Users will be able
    ...
    {
        if (empty($input['milex-object-id'])) {
            return $input;
        }

        if (!is_array($input['milex-object-id'])) {
            return $input;
        }

        foreach ($input['milex-object-id'] as $key => $milexObjectId) {
            $input['milex-object-id'][$key] = preg_replace(
                '/^contact:/',
                Contact::NAME.':',
                "$milexObjectId"
            );
