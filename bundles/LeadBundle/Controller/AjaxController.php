...
use Milex\LeadBundle\Form\Type\FilterPropertiesType;
...
use Milex\LeadBundle\Provider\FormAdjustmentsProviderInterface;
use Milex\LeadBundle\Segment\Stat\SegmentCampaignShare;
...
class AjaxController extends CommonAjaxController
{
    ...
    protected function getLeadIdsByFieldValueAction(Request $request)
    {
        ...
        if ($field && $value) {
            ...
            $dataArray['existsMessage'] = $this->translator->trans('milex.lead.exists.by.field').': ';
            ...
            foreach ($leads as $lead) {
                ...
                if (!$name) {
                    $name = $this->translator->trans('milex.lead.lead.anonymous');
                }

                $leadLink = $this->generateUrl('milex_contact_action', ['objectAction' => 'view', 'objectId' => $lead->getId()]);
    ...
    protected function loadSegmentFilterFormAction(Request $request): JsonResponse
    {
        ...
        $formAdjustmentsProvider = $this->get('milex.lead.provider.formAdjustments');

        /** @var ListModel $listModel */
        $listModel = $this->get('milex.lead.model.list');
        ...
        $formHtml = $this->renderView(
            'MilexLeadBundle:List:filterpropform.html.php',
            [
                'form' => $this->setFormTheme($form, 'MilexLeadBundle:List:filterpropform.html.php', []),
            ]
        );
    ...
    protected function updateSocialProfileAction(Request $request)
    {
        ...
        if (!empty($leadId)) {
            ...
            if (null !== $lead && $this->get('milex.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editown', $lead->getPermissionUser())) {
                ...
                if (empty($network) || empty($socialCount)) {
                    $dataArray['completeProfile'] = $this->renderView(
                        'MilexLeadBundle:Social:index.html.php',
                        [
    ...
    protected function clearSocialProfileAction(Request $request)
    {
        ...
        if (!empty($leadId)) {
            ...
            if (null !== $lead && $this->get('milex.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editown', $lead->getPermissionUser())) {
                $dataArray['success'] = 1;
                /** @var \Milex\PluginBundle\Helper\IntegrationHelper $helper */
    ...
    protected function updateTimelineAction(Request $request)
    {
        ...
        if (!empty($leadId)) {
            ...
            if (null !== $lead) {
                ...
                $session->set('milex.lead.'.$leadId.'.timeline.filters', $filter);
                ...
                $timeline = $this->renderView(
                    'MilexLeadBundle:Lead:history.html.php',
                    [
                        'events'       => $events,
    ...
    protected function getImportProgressAction(Request $request)
    {
        ...
        if ($this->get('milex.security')->isGranted('lead:leads:create')) {
            $session               = $this->get('session');
            $dataArray['progress'] = $session->get('milex.lead.import.progress', [0, 0]);
    ...
    protected function removeBounceStatusAction(Request $request)
    {
        ...
        if (!empty($dncId)) {
            ...
            $dnc = $this->getDoctrine()->getManager()->getRepository('MilexLeadBundle:DoNotContact')->findOneBy(
                [
                    'id' => $dncId,
                ]
            );
        ...
        if (!empty($maxId)) {
            ...
            if (!empty($count)) {
                ...
                $dataArray['leads'] = $this->factory->getTemplating()->render(
                    "MilexLeadBundle:Lead:{$template}.html.php",
                    [
    ...
    protected function updateLeadTagsAction(Request $request)
    {
        ...
        if (null !== $lead && $this->get('milex.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editother', $lead->getPermissionUser())) {
    ...
    protected function reorderAction(Request $request)
    {
        ...
        if (!empty($order)) {
            /** @var \Milex\LeadBundle\Model\FieldModel $model */
                ...
                switch ($leadFieldType) {
                    ...
                    case 'datetime':
                        if ('date' == $operator) {
                            ...
                            $options = array_merge(
                                [
                                    'custom' => $this->translator->trans('milex.campaign.event.timed.choice.custom'),
                                ],
                                $options
                            );
    ...
    protected function getCampaignShareStatsAction(Request $request)
    {
        ...
        $segmentCampaignShareService = $this->get('milex.lead.segment.stat.campaign.share');
