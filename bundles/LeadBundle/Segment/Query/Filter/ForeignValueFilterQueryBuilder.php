...
class ForeignValueFilterQueryBuilder extends BaseFilterQueryBuilder
{
    ...
    {
        return 'milex.lead.query.builder.foreign.value';
    }
    ...
    public function applyQuery(QueryBuilder $queryBuilder, ContactSegmentFilter $filter)
    {
        ...
        if (!is_null($filter->getWhere())) {
            $subQueryBuilder->andWhere(str_replace(str_replace(MILEX_TABLE_PREFIX, '', $filter->getTable()).'.', $tableAlias.'.', $filter->getWhere()));
        }
