//LeadBundle
Milex.companyOnLoad = function (container, response) {

    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'lead.company');
    }
}
Milex.leadOnLoad = function (container, response) {
    Milex.addKeyboardShortcut('a', 'Quick add a New Contact', function(e) {
    ...
    Milex.addKeyboardShortcut('t', 'Activate Table View', function(e) {
        mQuery('#table-view').click();
    }, 'contact pages');

    Milex.addKeyboardShortcut('c', 'Activate Card View', function(e) {
    ...
    if (timelineForm.length) {
        timelineForm.on('change', function() {
            ...
            e.preventDefault();
            Milex.refreshLeadTimeline(timelineForm);
        });
        ...
        Milex.leadTimelineOnLoad(container, response);
        Milex.leadAuditlogOnLoad(container, response);
    }
    ...
    if (auditlogForm.length) {
        auditlogForm.on('change', function() {
            ...
            e.preventDefault();
            Milex.refreshLeadAuditLog(auditlogForm);
        });
    }
    ...
    if (noteForm.length) {
        noteForm.on('change', function() {
            ...
            e.preventDefault();
            Milex.refreshLeadNotes(noteForm);
        });
    }

    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'lead.lead');
    }

    if (mQuery(container + ' #notes-container').length) {
        Milex.activateSearchAutocomplete('NoteFilter', 'lead.note');
    }
    ...
    if (mQuery('.lead-avatar-panel').length) {
        mQuery('.lead-avatar-panel .avatar-collapser a.arrow').on('click', function() {
            setTimeout(function() {
                ...
                Cookies.set('milex_lead_avatar_panel', status, {expires: 30});
            }, 500);
        });
    }
    ...
    mQuery(document).on('shown.bs.tab', 'a#load-lead-map', function (e) {
        leadMap = Milex.renderMap(mQuery('#place-container .vector-map'));
    });

    mQuery('a[data-toggle="tab"]').not('a#load-lead-map').on('shown.bs.tab', function (e) {
        if (leadMap.length) {
            Milex.destroyMap(leadMap);
            leadMap = [];
        }
    });

    Milex.initUniqueIdentifierFields();
    ...
    mQuery('#lead_tags_chosen input').keyup(function(el) {
        ...
        if (el.key === "Escape" && newTag !== '') {
            ...
            Milex.activateLabelLoadingIndicator(mQuery(selectElement).attr('id'));
            Milex.ajaxActionRequest('lead:addLeadTags', {tags: JSON.stringify(payload)}, function(response) {
                if (response.tags) {
...
Milex.leadTimelineOnLoad = function (container, response) {
...
Milex.leadAuditlogOnLoad = function (container, response) {
...
Milex.leadOnUnload = function(id) {
    if (typeof MilexVars.moderatedIntervals['leadListLiveUpdate'] != 'undefined') {
        Milex.clearModeratedInterval('leadListLiveUpdate');
    }

    if (typeof Milex.mapObjects !== 'undefined') {
        delete Milex.mapObjects;
    }
};

Milex.getLeadId = function() {
    return mQuery('input#leadId').val();
}

Milex.leadlistOnLoad = function(container, response) {
    ...
    mQuery('#campaign-share-tab').hover(function () {
        if (Milex.shareTableLoaded != true) {
            Milex.loadAjaxColumn('campaign-share-stat', 'lead:getCampaignShareStats', 'afterStatsLoad');
            Milex.shareTableLoaded = true;
        }
    })

    Milex.afterStatsLoad = function () {
        Milex.sortTableByColumn('#campaign-share-table', '.campaign-share-stat', true)
    }


    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'lead.list');
    }
    ...
    if (mQuery('#' + prefix + '_filters').length) {
        mQuery('#available_segment_filters').on('change', function() {
            if (mQuery(this).val()) {
                Milex.addLeadListFilter(mQuery(this).val(),mQuery('option:selected',this).data('field-object'));
                mQuery(this).val('');
    ...
    if (segmentContactForm.length) {
        segmentContactForm.on('change', function() {
            ...
            e.preventDefault();
            Milex.refreshSegmentContacts(segmentContactForm);
        });
    }

    jQuery(document).ajaxComplete(function(){
        Milex.ajaxifyForm('daterange');
    });

    Milex.attachJsUiOnFilterForms();
};

/**
 * Trigger event so plugins could attach other JS magic to the form.
 */
Milex.triggerOnPropertiesFormLoadedEvent = function(selector, filterValue) {
    mQuery('#leadlist_filters').trigger('filter.properties.form.loaded', [selector, filterValue]);
};

Milex.attachJsUiOnFilterForms = function() {
    mQuery('#leadlist_filters').on('filter.properties.form.loaded', function(event, selector, filterValue) {
        Milex.activateChosenSelect(selector + '_properties select');
        ...
        if (fieldType === 'lookup') {
            Milex.activateLookupTypeahead(filterFieldEl.parent());
            ...
            if (fieldCallback && typeof Milex[fieldCallback] === 'function') {
                var fieldOptions = displayFieldEl.attr('data-field-list');
                Milex[fieldCallback](selector.replace('#', '') + '_properties_display', fieldAlias, fieldOptions);
            }
        } 
    });
    ...
    mQuery('#leadlist_filters .panel').each(function() {
        Milex.triggerOnPropertiesFormLoadedEvent('#' + mQuery(this).attr('id'));
    });
};

Milex.reorderSegmentFilters = function() {
    ...
    mQuery('#' + prefix + '_filters .panel').each(function() {
        Milex.updateFilterPositioning(mQuery(this).find('select.glue-select').first());
        mQuery(this).find('[id^="' + prefix + '_filters_"]').each(function() {
            ...
            if (mQuery(this).is('select') && suffix == "filter") {
                Milex.destroyChosen(mQuery(this));
                Milex.activateChosenSelect(mQuery(this));
            }
        });
...
Milex.convertLeadFilterInput = function(el) {
    ...
    Milex.loadFilterForm(filterNum, fieldObject.val(), fieldAlias.val(), operatorSelect.val(), function(propertiesFields) {
        var selector = '#leadlist_filters_'+filterNum;
        mQuery(selector+'_properties').html(propertiesFields);

        Milex.triggerOnPropertiesFormLoadedEvent(selector, filterValue);
    });

    Milex.setProcessorForFilterValue(filterId, operatorSelect.val());
};

Milex.setFilterValuesProcessor = function () {
    mQuery('.filter-operator').each(function (index) {
        let filterId = "#" + mQuery('.filter-value').eq(index).attr('id');
        Milex.setProcessorForFilterValue(filterId, mQuery(this).val())
    });
};

Milex.setProcessorForFilterValue = function (filterId, operator) {
 ...
 */
Milex.updateLookupListFilter = function(field, item) {
    if (item && item.id) {
...
Milex.activateSegmentFilterTypeahead = function(displayId, filterId, fieldOptions, mQueryObject) {
    ...
    Milex.activateFieldTypeahead(displayId, filterId, [], mQuery('#' + displayId).data('action') || 'lead:fieldList');

    mQuery = mQueryBackup;
};

Milex.loadFilterForm = function(filterNum, fieldObject, fieldAlias, operator, resultHtml, search = null) {
    mQuery.ajax({
        showLoadingBar: true,
        url: milexAjaxUrl,
        type: 'POST',
        ...
        success: function (response) {
            ...
            if (fieldAlias == 'lead_asset_download') {
                Milex.handleAssetDownloadSearch(filterNum, fieldObject, fieldAlias, operator, resultHtml, search);
            }
        },
...
Milex.addLeadListFilter = function (elId, elObj) {
    ...
    Milex.convertLeadFilterInput('#' + filterIdBase + 'operator');

    // Reposition if applicable
    Milex.updateFilterPositioning(mQuery('#' + filterIdBase + 'glue'));
};

Milex.leadfieldOnLoad = function (container) {
    if (mQuery(container + ' .leadfield-list').length) {
        ...
        mQuery(container + ' .leadfield-list tbody').sortable({
            ...
            stop: function(e, ui) {
                ...
                mQuery.ajax({
                    type: "POST",
                    url: milexAjaxUrl + "?action=lead:reorder&limit=" + mQuery('.pagination-limit').val() + '&page=' + mQuery('.pagination li.active a span').first().text(),
    ...
    if (mQuery(container + ' form[name="leadfield"]').length) {
        Milex.updateLeadFieldProperties(mQuery('#leadfield_type').val(), true);
    }

};

Milex.updateLeadFieldProperties = function(selectedVal, onload) {
    ...
    if (defaultValueField.hasClass('calendar-activated')) {
        ...
        Milex.destroyChosen(defaultValueField);
    }
    ...
    if (selectedVal === 'datetime' || selectedVal === 'date' || selectedVal === 'time') {
        Milex.activateDateTimeInputs('#leadfield_defaultValue', selectedVal);
    } else if (isSelect) {
       Milex.activateChosenSelect('#leadfield_defaultValue');
    }
};

Milex.updateLeadFieldBooleanLabels = function(el, label) {
    mQuery('#leadfield_defaultValue_' + label).parent().find('span').text(
        mQuery(el).val()
    );
};

Milex.refreshLeadSocialProfile = function(network, leadId, event) {
    ...
    Milex.setFilterValuesProcessor();
};

Milex.clearLeadSocialProfile = function(network, leadId, event) {
    Milex.startIconSpinOnEvent(event);
...
Milex.refreshLeadAuditLog = function(form) {
    Milex.postForm(mQuery(form), function (response) {
...
Milex.refreshLeadTimeline = function(form) {
    Milex.postForm(mQuery(form), function (response) {
...
Milex.refreshLeadNotes = function(form) {
    Milex.postForm(mQuery(form), function (response) {
...
Milex.refreshSegmentContacts = function(form) {
    Milex.postForm(mQuery(form), function (response) {
...
Milex.toggleLeadList = function(toggleId, leadId, listId) {
    ...
    Milex.toggleLeadSwitch(toggleId, query, action);
};

Milex.togglePreferredChannel = function(channel) {
    if (channel === 'all') {
        ...
        channelsForm.find('tbody input:checkbox').each(function() {
            if (this.checked != status) {
                this.checked = status;
                Milex.setPreferredChannel(this.value);
            }
        });
    } else {
        Milex.setPreferredChannel(channel);
    }
};

Milex.setPreferredChannel = function(channel) {
...
Milex.toggleCompanyLead = function(toggleId, leadId, companyId) {
    var action = mQuery('#' + toggleId).hasClass('fa-toggle-on') ? 'remove' : 'add';
    var query = "action=lead:toggleCompanyLead&leadId=" + leadId + "&companyId=" + companyId + "&companyAction=" + action;
    Milex.toggleLeadSwitch(toggleId, query, action);
};

Milex.toggleLeadCampaign = function(toggleId, leadId, campaignId) {
    var action = mQuery('#' + toggleId).hasClass('fa-toggle-on') ? 'remove' : 'add';
...
Milex.toggleLeadSwitch = function(toggleId, query, action) {
...
Milex.leadNoteOnLoad = function (container, response) {
    if (response.noteHtml) {
        ...
        Milex.makeModalsAlive(mQuery(el + " *[data-toggle='ajaxmodal']"));
        Milex.makeConfirmationsAlive(mQuery(el+' a[data-toggle="confirmation"]'));
        Milex.makeLinksAlive(mQuery(el + " a[data-toggle='ajax']"));
...
Milex.showSocialMediaImageModal = function(imgSrc) {
    mQuery('#socialImageModal img').attr('src', imgSrc);
    mQuery('#socialImageModal').modal('show');
};

Milex.leadImportOnLoad = function (container, response) {
    if (!mQuery('#leadImportProgress').length) {
        Milex.clearModeratedInterval('leadImportProgress');
    } else {
        Milex.setModeratedInterval('leadImportProgress', 'reloadLeadImportProgress', 3000);
    }
};

Milex.reloadLeadImportProgress = function() {
    if (!mQuery('#leadImportProgress').length) {
        Milex.clearModeratedInterval('leadImportProgress');
    } else {
        // Get progress separate so there's no delay while the import batches
        Milex.ajaxActionRequest('lead:getImportProgress', {}, function(response) {
        ...
        mQuery.ajax({
            ...
            success: function(response) {
                Milex.moderatedIntervalCallbackIsComplete('leadImportProgress');
...
Milex.removeBounceStatus = function (el, dncId) {
    mQuery(el).removeClass('fa-times').addClass('fa-spinner fa-spin');

    Milex.ajaxActionRequest('lead:removeBounceStatus', 'id=' + dncId, function() {
        mQuery('#bounceLabel' + dncId).tooltip('destroy');
        mQuery('#bounceLabel' + dncId).fadeOut(300, function() { mQuery(this).remove(); });
    });
};

Milex.toggleLiveLeadListUpdate = function () {
    if (typeof MilexVars.moderatedIntervals['leadListLiveUpdate'] == 'undefined') {
        Milex.setModeratedInterval('leadListLiveUpdate', 'updateLeadList', 5000);
...
Milex.updateLeadList = function () {
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            ...
            Milex.moderatedIntervalCallbackIsComplete('leadListLiveUpdate');
        },
        error: function (request, textStatus, errorThrown) {
            Milex.processAjaxError(request, textStatus, errorThrown);
...
Milex.toggleAnonymousLeads = function() {
    ...
    Milex.setSearchFilter(null, 'list-search', searchValue);
};

Milex.getLeadEmailContent = function (el) {
    var id = (mQuery.type( el ) === "string") ? el : mQuery(el).attr('id');
    Milex.activateLabelLoadingIndicator(id);

    var inModal = mQuery('#'+id).closest('modal').length;
    if (inModal) {
        mQuery('#MilexSharedModal .btn-primary').prop('disabled', true);
    }

    Milex.ajaxActionRequest('lead:getEmailTemplate', {'template': mQuery(el).val()}, function(response) {
        if (inModal) {
            mQuery('#MilexSharedModal .btn-primary').prop('disabled', false);
        }
        var idPrefix = id.replace('templates', '');
        var bodyEl = (mQuery('#'+idPrefix+'message').length) ? '#'+idPrefix+'message' : '#'+idPrefix+'body';

        if (Milex.getActiveBuilderName() === 'legacy') {
...
Milex.updateLeadTags = function () {
    Milex.activateLabelLoadingIndicator('lead_tags_tags');
    var formData = mQuery('form[name="lead_tags"]').serialize();
    Milex.ajaxActionRequest('lead:updateLeadTags', formData, function(response) {
        if (response.tags) {
...
Milex.createLeadTag = function (el) {
    var newFound = false;
    ...
    Milex.ajaxActionRequest('lead:addLeadTags', {tags: tags}, function(response) {
        if (response.tags) {
...
Milex.createLeadUtmTag = function (el) {
    var newFound = false;
    mQuery('#' + mQuery(el).attr('id') + ' :selected').each(function(i, selected) {
        if (!mQuery.isNumeric(mQuery(selected).val())) {
            newFound = true;
    ...
    Milex.ajaxActionRequest('lead:addLeadUtmTags', {utmtags: utmtags}, function(response) {
        if (response.tags) {
...
Milex.leadBatchSubmit = function() {
    if (Milex.batchActionPrecheck()) {
    ...
    mQuery('#MilexSharedModal').modal('hide');

    return false;
};

Milex.updateLeadFieldValues = function (field) {
    mQuery('.condition-custom-date-row').hide();
    Milex.updateFieldOperatorValue(field, 'lead:updateLeadFieldValues', Milex.updateLeadFieldValueOptions, [true]);
};

Milex.updateLeadFieldValueOptions = function (field, updating) {
...
Milex.toggleTimelineMoreVisiblity = function (el) {
    if (mQuery(el).is(':visible')) {
        mQuery(el).slideUp('fast');
        mQuery(el).next().text(milexLang['showMore']);
    } else {
        mQuery(el).slideDown('fast');
        mQuery(el).next().text(milexLang['hideMore']);
    }
};

Milex.displayUniqueIdentifierWarning = function (el) {
    if (mQuery(el).val() === "0") {
...
Milex.initUniqueIdentifierFields = function() {
    ...
    if (uniqueFields.length) {
        uniqueFields.on('change', function() {
            ...
            Milex.ajaxActionRequest('lead:getLeadIdsByFieldValue', request, function(response) {
...
Milex.updateFilterPositioning = function (el) {
    var $el       = mQuery(el);
...
Milex.setAsPrimaryCompany = function (companyId,leadId){
    Milex.ajaxActionRequest('lead:setAsPrimaryCompany', {'companyId': companyId, 'leadId': leadId}, function(response) {
        if (response.success) {
...
Milex.handleAssetDownloadSearch = function(filterNum, fieldObject, fieldAlias, operator, resultHtml, search) {
    ...
    assetDownloadFilter.on('chosen:no_results', function () {
        ...
        mQuery(this).data('timer', setTimeout(function () {
            assetDownloadInput.width('auto').prop('disabled', true).val(Milex.translate('milex.core.lookup.loading_data'));
            Milex.loadFilterForm(filterNum, fieldObject, fieldAlias, operator, resultHtml, search)
        }, 1000, search))
    });
    ...
    if (mQuery('#leadlist_filters_' + filterNum + '_properties_filter option').length === 0 ) {
        assetDownloadInput.val(milexLang['chosenNoResults']);
    }
    else if (search !== null) {
...
Milex.listOnLoad = function(container, response) {
    Milex.lazyLoadContactListOnSegmentDetail();
};

Milex.lazyLoadContactListOnSegmentDetail = function() {
