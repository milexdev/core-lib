...
use Milex\LeadBundle\Event\LeadChangeCompanyEvent;
use Milex\LeadBundle\Exception\UniqueFieldNotFoundException;
use Milex\LeadBundle\Form\Type\CompanyType;
...
class CompanyModel extends CommonFormModel implements AjaxLookupModelInterface
{
     ...
     * {@inheritdoc}
     *
     * @return \Milex\LeadBundle\Entity\CompanyRepository
     */
    public function getRepository()
    {
        $repo =  $this->em->getRepository('MilexLeadBundle:Company');
        if (!$this->repoSetup) {
            $this->repoSetup = true;
     ...
     * {@inheritdoc}
     *
     * @return \Milex\LeadBundle\Entity\CompanyLeadRepository
     */
    public function getCompanyLeadRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:CompanyLead');
    }
    ...
    public function getUserCompanies()
    {
        ...
        return $this->em->getRepository('MilexLeadBundle:Company')->getCompanies($user);
    }
    ...
    public function addLeadToCompany($companies, $lead)
    {
        ...
        if (!$lead instanceof Lead) {
            ...
            $lead   = $this->em->getReference('MilexLeadBundle:Lead', $leadId);
        }
    ...
    public function extractCompanyDataFromImport(array &$mappedFields, array &$data)
    {
        ...
        foreach ($mappedFields as $milexField => $importField) {
            foreach ($internalFields as $entityField) {
                if ($entityField['alias'] === $milexField) {
                    $companyData[$importField]   = $data[$importField];
                    $companyFields[$milexField] = $importField;
                    unset($data[$importField]);
                    unset($mappedFields[$milexField]);
                    break;
                }
            }
        }
    ...
    public function import($fields, $data, $owner = null, $skipIfExists = false)
    {
        ...
        if (null === $company) {
            throw new \Exception($this->translator->trans('milex.company.error.notfound', [], 'flashes'));
        }
    ...
    public function importCompany($fields, $data, $owner = null, $persist = true, $skipIfExists = false)
    {
        ...
        if (!empty($fields['createdByUser']) && !empty($data[$fields['createdByUser']])) {
            $userRepo      = $this->em->getRepository('MilexUserBundle:User');
        ...
        if (null !== $owner) {
            $company->setOwner($this->em->getReference('MilexUserBundle:User', $owner));
        }
    ...
    private function updateContactAfterPrimaryCompanyWasRemoved(Lead $lead): void
    {
        ...
        $lead->addUpdatedField('company', $primaryCompanyName)
            ->setDateModified(new \DateTime());
        $this->em->getRepository('MilexLeadBundle:Lead')->saveEntity($lead);
    }
}
