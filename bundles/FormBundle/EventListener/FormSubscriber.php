...
use Milex\FormBundle\Exception\ValidationException;
...
class FormSubscriber implements EventSubscriberInterface
{
    ...
    public function onFormBuilder(Events\FormBuilderEvent $event)
    {
        $event->addSubmitAction('form.email', [
            ...
            'label'              => 'milex.form.action.sendemail',
            'description'        => 'milex.form.action.sendemail.descr',
            'formType'           => SubmitActionEmailType::class,
            'formTheme'          => 'MilexFormBundle:FormTheme\SubmitAction',
            'formTypeCleanMasks' => [
                'message' => 'html',
            ],
        ...
        $event->addSubmitAction('form.repost', [
            'group'              => 'milex.form.actions',
            'label'              => 'milex.form.action.repost',
            'description'        => 'milex.form.action.repost.descr',
    ...
    public function onFormSubmitActionRepost(Events\SubmissionEvent $event)
    {
        ...
        $matchedFields = [];
        $payload       = [
            'milex_contact' => $lead->getProfileFields(),
            'milex_form'    => [
                'id'   => $post['formId'],
                'name' => $post['formName'],
                'url'  => $post['return'],
        ...
        try {
            ...
            if (!empty($email)) {
                // Remove Milex values and password fields
                ...
                $post['milex_contact'] = array_filter($payload['milex_contact']);
                $post['milex_form']    = $payload['milex_form'];
                ...
                $this->mailer->setSubject(
                    $this->translator->trans('milex.form.action.repost.failed_subject', ['%form%' => $submission->getForm()->getName()])
                );
                $this->mailer->setBody(
                    $this->translator->trans(
                        'milex.form.action.repost.failed_message',
                        [
    ...
    private function parseResponse(Response $response, array $matchedFields = [])
    {
        ...
        if (is_array($body)) {
            if (isset($body['error'])) {
                ...
                $error          = $this->translator->trans('milex.form.action.repost.validation_failed');
                $formViolations = $body['violations'];

                // Ensure the violations match up to Milex's
                $violations = [];
