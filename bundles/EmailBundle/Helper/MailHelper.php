...
use Milex\EmailBundle\Exception\PartialEmailSendFailure;
use Milex\EmailBundle\Swiftmailer\Exception\BatchQueueMaxException;
use Milex\EmailBundle\Swiftmailer\Message\MilexMessage;
use Milex\EmailBundle\Swiftmailer\Transport\SpoolTransport;
use Milex\EmailBundle\Swiftmailer\Transport\TokenTransportInterface;
...
class MailHelper
{
    ...
    public function __construct(MilexFactory $factory, \Swift_Mailer $mailer, $from = null)
    {
        ...
        if (method_exists($this->transport, 'setMilexFactory')) {
            $this->transport->setMilexFactory($factory);
        }
    ...
    /**
     * Mirrors previous MilexFactory functionality.
     *
    ...
    public function send($dispatchSendEvent = false, $isQueueFlush = false)
    {
        ...
        if (empty($this->fatal)) {
            ...
            if (!empty($this->assets)) {
                /** @var \Milex\AssetBundle\Entity\Asset $asset */
     ...
     *                                  FULL_RESET         creates a new MilexMessage instance and resets errors
     *                                  DO_NOTHING         leaves the current errors array and MilexMessage instance intact
     *                                  NOTHING_IF_FAILED  leaves the current errors array MilexMessage instance intact if it fails, otherwise reset_to
    ...
    /**
     * Get a MilexMessage/Swift_Message instance.
     *
     * @return bool|MilexMessage
     */
    public function getMessageInstance()
    {
        try {
            return $this->tokenizationEnabled ? MilexMessage::newInstance() : (new \Swift_Message());
        } catch (\Exception $e) {
            $this->logError($e);
    ...
    public function setEmail(Email $email, $allowBcc = true, $slots = [], $assetAttachments = [], $ignoreTrackingPixel = false)
    {
        ...
        // Process emails created by Milex v1
        ...
        if ($listUnsubscribeHeader) {
            if (!empty($headers['List-Unsubscribe'])) {
                if (false === strpos($headers['List-Unsubscribe'], $listUnsubscribeHeader)) {
                    // Ensure Milex's is always part of this header
    ...
    {
        if ($this->idHash) {
            $url = $this->factory->getRouter()->generate('milex_email_unsubscribe', ['idHash' => $this->idHash], UrlGeneratorInterface::ABSOLUTE_URL);
    ...
    protected function logError($error, $context = null)
    {
        if ($error instanceof \Exception) {
            ...
            $error            = ('dev' === MILEX_ENV) ? (string) $error : $errorMessage;
     ...
     * @param $url
     *
     * @return \Milex\PageBundle\Entity\Redirect|object|null
     */
    public function getTrackableLink($url)
    {
        ...
        if ($this->email) {
            ...
            /** @var \Milex\PageBundle\Model\TrackableModel $trackableModel */
        ...
        /** @var \Milex\PageBundle\Model\RedirectModel $redirectModel */
    ...
    public function createEmailStat($persist = true, $emailAddress = null, $listId = null)
    {
        ...
        if (null !== $this->lead) {
            try {
                $stat->setLead($this->factory->getEntityManager()->getReference('MilexLeadBundle:Lead', $this->lead['id']));
        ...
        if (null !== $listId) {
            try {
                $stat->setList($this->factory->getEntityManager()->getReference('MilexLeadBundle:LeadList', $listId));
            } catch (ORMException $exception) {
                // keep IDE happy
            }
        }
        ...
        /** @var \Milex\EmailBundle\Model\EmailModel $emailModel */
        ...
        if (isset($this->copies[$id])) {
            try {
                $stat->setStoredCopy($this->factory->getEntityManager()->getReference('MilexEmailBundle:Copy', $this->copies[$id]));
            } catch (ORMException $exception) {
                // keep IDE happy
            }
        }
    ...
    {
        /** @var \Milex\EmailBundle\MonitoredEmail\Mailbox $mailboxHelper */
     ...
     *
     * @deprecated 2.11.0 to be removed in 3.0; use Milex\EmailBundle\Helper\EmailValidator
     *
     * @param $address
     *
