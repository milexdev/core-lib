...
use Milex\EmailBundle\Model\TransportCallback;
...
class MandrillTransport extends AbstractTokenHttpTransport implements CallbackTransportInterface
{
    ...
    protected function getPayload()
    {
        ...
        $milexTokens = $mandrillMergeVars = $mandrillMergePlaceholders = [];

        // Mandrill uses *|PLACEHOLDER|* for tokens so Milex's need to be replaced
        if (!empty($metadata)) {
            $metadataSet  = reset($metadata);
            $tokens       = (!empty($metadataSet['tokens'])) ? $metadataSet['tokens'] : [];
            $milexTokens = array_keys($tokens);

            $mandrillMergeVars = $mandrillMergePlaceholders = [];
            foreach ($milexTokens as $token) {
                $mandrillMergeVars[$token]         = strtoupper(preg_replace('/[^a-z0-9]+/i', '', $token));
                $mandrillMergePlaceholders[$token] = '*|'.$mandrillMergeVars[$token].'|*';
            }
        }

        $message = $this->messageToArray($milexTokens, $mandrillMergePlaceholders, true);
