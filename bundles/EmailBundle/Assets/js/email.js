/** EmailBundle **/
Milex.emailOnLoad = function (container, response) {
    Milex.internalDynamicContentItemCreateListeners = [];
    Milex.internalDynamicContentFilterCreateListeners = [];

    if (mQuery('#emailform_plainText').length) {
        // @todo initiate the token dropdown
        var plaintext = mQuery('#emailform_plainText');

        Milex.initAtWho(plaintext, plaintext.attr('data-token-callback'));
        Milex.initSelectTheme(mQuery('#emailform_template'));
        Milex.initEmailDynamicContent();

        Milex.prepareVersioning(
            function (content) {
                console.log('undo');
            },
            function (content) {
        ...
        if (response && response.inBuilder) {
            Milex.isInBuilder = true;
            Milex.launchBuilder('emailform');
            Milex.processBuilderErrors(response);
        }
    } else if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'email');
    }
    ...
    if (mQuery('#emailGraphStats').length) {
        ...
        mQuery("#emailGraphStats").load(graphUrl, function () {
            Milex.renderCharts();
            Milex.initDateRangePicker('#emailGraphStats #daterange_date_from', '#emailGraphStats #daterange_date_to');
        });
    }
};

Milex.emailOnUnload = function(id) {
    if (id === '#app-content') {
        delete Milex.listCompareChart;
    }

    if (typeof Milex.ajaxActionXhrQueue !== 'undefined') {
        delete Milex.ajaxActionXhrQueue['email:getEmailCountStats'];
    }
};

Milex.insertEmailBuilderToken = function(editorId, token) {
    var editor = Milex.getEmailBuilderEditorInstances();
    editor[instance].insertText(token);
};

Milex.getEmailAbTestWinnerForm = function(abKey) {
    ...
    Milex.activateLabelLoadingIndicator('emailform_variantSettings_winnerCriteria');
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.html != 'undefined') {
                ...
                if (response.html != '') {
                    Milex.onPageLoad('#emailform_variantSettings_properties', response);
                }
            }
        },
...
Milex.submitSendForm = function () {
    Milex.dismissConfirmation();
    mQuery('.btn-send').prop('disabled', true);
    mQuery('form[name=\'batch_send\']').submit();
};

Milex.emailSendOnLoad = function (container, response) {
    if (mQuery('.email-send-progress').length) {
        if (!mQuery('#emailSendProgress').length) {
            Milex.clearModeratedInterval('emailSendProgress');
        } else {
            Milex.setModeratedInterval('emailSendProgress', 'sendEmailBatch', 2000);
        }
    }
};

Milex.emailSendOnUnload = function () {
    if (mQuery('.email-send-progress').length) {
        Milex.clearModeratedInterval('emailSendProgress');
        if (typeof Milex.sendEmailBatchXhr != 'undefined') {
            Milex.sendEmailBatchXhr.abort();
            delete Milex.sendEmailBatchXhr;
        }
    }
};

Milex.sendEmailBatch = function () {
    ...
    Milex.sendEmailBatchXhr = Milex.ajaxActionRequest('email:sendBatch', data, function (response) {
        ...
        Milex.moderatedIntervalCallbackIsComplete('emailSendProgress');
    });
};

Milex.autoGeneratePlaintext = function() {
...
Milex.getTotalAttachmentSize = function() {
    var assets = mQuery('#emailform_assetAttachments').val();
    if (assets) {
        assets = {
            'assets': assets
        };
        Milex.ajaxActionRequest('email:getAttachmentsSize', assets, function(response) {
...
Milex.standardEmailUrl = function(options) {
 ...
 * @param origin
 */
Milex.disabledEmailAction = function(opener, origin) {
...
Milex.initEmailDynamicContent = function() {
    if (mQuery('#dynamic-content-container').length) {
        ...
        mQuery('#addNewDynamicContent').on('click', function (e) {
            ...
            Milex.createNewDynamicContentItem();
        });

        Milex.initDynamicContentItem();
    }
};

Milex.createNewDynamicContentItem = function(jQueryVariant) {
    ...
    if (textarea.hasClass('legacy-builder')) {
        textarea.froalaEditor(mQuery.extend({}, Milex.basicFroalaOptions, {
    ...
    if (Milex.internalDynamicContentItemCreateListeners) {
        Milex.internalDynamicContentItemCreateListeners.forEach(function(callback) {
            callback(textarea);
        });
    }
    ...
    Milex.updateDynamicContentDropdown();

    Milex.initDynamicContentItem(tabId, mQuery, tokenName);

    return tabId;
};

Milex.dynamicContentAddNewItemListener = function(callback) {
    Milex.internalDynamicContentItemCreateListeners.push(callback);
}

Milex.createNewDynamicContentFilter = function(el, jQueryVariant) {
    // To support the parent.mQuery from the builder
    var mQuery = (typeof jQueryVariant != 'undefined') ? jQueryVariant : window.mQuery;
    ...
    var availableFilters = filterContainer.find('select[data-milex="available_filters"]');
    var altTextarea      = filterContainer.find('.editor');
    var removeButton     = filterContainer.find('.remove-item');

    Milex.activateChosenSelect(availableFilters, false, mQuery);

    availableFilters.on('change', function() {
        var $this = mQuery(this);

        if ($this.val()) {
            Milex.addDynamicContentFilter($this.val(), mQuery);
            $this.val('');
    ...
    if (altTextarea.hasClass('legacy-builder')) {
        altTextarea.froalaEditor(mQuery.extend({}, Milex.basicFroalaOptions, {
            // Set custom buttons with separator between them.
            toolbarSticky: false,
            toolbarButtons: ['undo', 'redo', '|', 'bold', 'italic', 'underline', 'paragraphFormat', 'fontFamily', 'fontSize', 'color', 'align', 'formatOL', 'formatUL', 'quote', 'clearFormatting', 'token', 'insertLink', 'insertImage', 'insertTable', 'html', 'fullscreen'],
            heightMin: 100
    ...
    if (Milex.internalDynamicContentFilterCreateListeners) {
        Milex.internalDynamicContentFilterCreateListeners.forEach(function(callback) {
            callback(altTextarea);
        });
    }

    Milex.initRemoveEvents(removeButton, mQuery);
...
Milex.dynamicContentAddNewFilterListener = function(callback) {
    Milex.internalDynamicContentFilterCreateListeners.push(callback);
}

Milex.initDynamicContentItem = function (tabId, jQueryVariant, tokenName) {
    // To support the parent.mQuery from the builder
    var mQuery = (typeof jQueryVariant != 'undefined') ? jQueryVariant : window.mQuery;
    ...
    $el.find('.addNewDynamicContentFilter').on('click', function (e) {
        ...
        Milex.createNewDynamicContentFilter(this);
    });
    ...
    $el.find('select[data-milex="available_filters"]').on('change', function() {
        var $this = mQuery(this);
    ...
    Milex.initRemoveEvents($el.find('.remove-item'), mQuery);
};

Milex.updateDynamicContentDropdown = function () {
    var options = [];
...
Milex.initRemoveEvents = function (elements, jQueryVariant) {
    var mQuery = (typeof jQueryVariant != 'undefined') ? jQueryVariant : window.mQuery;
...
Milex.addDynamicContentFilter = function (selectedFilter, jQueryVariant) {
    var mQuery = (typeof jQueryVariant != 'undefined') ? jQueryVariant : window.mQuery;
    ...
    var selectedOption  = mQuery('option[data-milex="available_' + selectedFilter + '"]').first();
    ...
    Milex.initRemoveEvents(activeDynamicContentFilterContainer.find("a.remove-selected"), mQuery);
    ...
    if (isSpecial) {
        ...
        if (fieldCallback && typeof Milex[fieldCallback] == 'function') {
            fieldOptions = selectedOption.data("field-list");
            Milex[fieldCallback](filterIdBase + '_filter', selectedFilter, fieldOptions);
        }
        ...
        if (fieldCallback && typeof Milex[fieldCallback] == 'function') {
            fieldOptions = selectedOption.data("field-list");
            Milex[fieldCallback](filterIdBase + '_display', selectedFilter, fieldOptions, mQuery);
        }
    } else {
    ...
    Milex.convertDynamicContentFilterInput('#' + filterIdBase + '_operator', mQuery);
};

Milex.convertDynamicContentFilterInput = function(el, jQueryVariant) {
    var mQuery = (typeof jQueryVariant != 'undefined') ? jQueryVariant : window.mQuery;
    ...
    if (filterEl.is('select')) {
        ...
        if (multiple && !isMultiple) {
            ...
            placeholder = milexLang['chosenChooseMore'];
            ...
            placeholder = milexLang['chosenChooseOne'];
        }
        ...
        Milex.destroyChosen(filterEl);

        filterEl.attr('data-placeholder', placeholder);

        Milex.activateChosenSelect(filterEl, false, mQuery);
    }
};
