...
use Milex\EmailBundle\Swiftmailer\Spool\DelegatingSpool;
use Milex\EmailBundle\Swiftmailer\Transport\SpoolTransport;
use Milex\EmailBundle\Tests\Helper\Transport\BatchTransport;
use Milex\EmailBundle\Tests\Helper\Transport\BcInterfaceTokenTransport;
use Milex\EmailBundle\Tests\Helper\Transport\SmtpTransport;
...
{
    /**
     * @var MilexFactory|\PHPUnit\Framework\MockObject\MockObject
     */
    private $mockFactory;
    ...
    protected function setUp(): void
    {
        ...
        $this->mockFactory           = $this->createMock(MilexFactory::class);
    ...
    {
        $parameterMap = [
            ['mailer_reply_to_email', false, 'admin@milex.com'],
        ];
        ...
        $replyTo = key((array) $mailer->message->getReplyTo());
        // Expect from address in reply to
        $this->assertEquals('admin@milex.com', $replyTo);
    }
    ...
    {
        $parameterMap = [
            ['mailer_custom_headers', [], ['X-Milex-Test' => 'test', 'X-Milex-Test2' => 'test']],
        ];
        ...
        foreach ($headers as $header) {
            if (false !== strpos($header->getFieldName(), 'X-Milex-Test')) {
    ...
    public function testGlobalHeadersAreIgnoredIfEmailEntityIsSet()
    {
        ...
        $headers = $mailer->message->getHeaders()->getAll();
        foreach ($headers as $header) {
            $this->assertFalse(strpos($header->getFieldName(), 'X-Milex-Test'), 'System headers were not supposed to be set');
        }
    }
    ...
    public function testEmailHeadersAreSet()
    {
        ...
        $email->setSubject('Test');
        $email->setCustomHtml('{signature}');
        $email->setHeaders(['X-Milex-Test3' => 'test2', 'X-Milex-Test4' => 'test2']);
        $mailer->setEmail($email);
        $mailer->send();
