...
use Milex\ApiBundle\Serializer\Driver\ApiMetadataDriver;
use Milex\AssetBundle\Entity\Asset;
use Milex\CoreBundle\Doctrine\Mapping\ClassMetadataBuilder;
use Milex\CoreBundle\Entity\DynamicContentEntityTrait;
use Milex\CoreBundle\Entity\FormEntity;
use Milex\CoreBundle\Entity\TranslationEntityInterface;
use Milex\CoreBundle\Entity\TranslationEntityTrait;
use Milex\CoreBundle\Entity\VariantEntityInterface;
use Milex\CoreBundle\Entity\VariantEntityTrait;
use Milex\CoreBundle\Helper\EmojiHelper;
use Milex\FormBundle\Entity\Form;
use Milex\LeadBundle\Entity\LeadList;
use Milex\LeadBundle\Form\Validator\Constraints\LeadListAccess;
...
class Email extends FormEntity implements VariantEntityInterface, TranslationEntityInterface
{
    ...
    /**
     * @var \Milex\CategoryBundle\Entity\Category
     **/
    private $category;
    ...
    /**
     * @var \Milex\FormBundle\Entity\Form
     */
    private $unsubscribeForm;

    /**
     * @var \Milex\PageBundle\Entity\Page
     */
    ...
    public static function loadValidatorMetadata(ClassMetadata $metadata)
    {
        ...
        $metadata->addPropertyConstraint(
            ...
            new NotBlank(
                [
                    'message' => 'milex.core.subject.required',
                ]
            )
        );
        ...
        $metadata->addConstraint(new Callback([
            'callback' => function (Email $email, ExecutionContextInterface $context) {
                ...
                if ('list' == $type && null == $translationParent) {
                    ...
                    $violations = $validator->validate(
                        ...
                        [
                            ...
                            new NotBlank(
                                [
                                    'message' => 'milex.lead.lists.required',
                                ]
                            ),
                        ]
                    );
                ...
                if ($email->isVariant()) {
                    ...
                    if ($total > 100) {
                        $context->buildViolation('milex.core.variant_weights_invalid')
