...
use Milex\CoreBundle\Helper\Chart\BarChart;
...
use Milex\CoreBundle\Model\BuilderModelTrait;
use Milex\CoreBundle\Model\FormModel;
use Milex\CoreBundle\Model\TranslationModelTrait;
use Milex\CoreBundle\Model\VariantModelTrait;
...
use Milex\EmailBundle\Event\EmailEvent;
...
use Milex\EmailBundle\Helper\StatsCollectionHelper;
...
use Milex\LeadBundle\Model\DoNotContact as DNC;
...
class EmailModel extends FormModel implements AjaxLookupModelInterface
{
     ...
     * {@inheritdoc}
     *
     * @return \Milex\EmailBundle\Entity\EmailRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('MilexEmailBundle:Email');
    }

    /**
     * @return \Milex\EmailBundle\Entity\StatRepository
     */
    public function getStatRepository()
    {
        return $this->em->getRepository('MilexEmailBundle:Stat');
    }

    /**
     * @return \Milex\EmailBundle\Entity\CopyRepository
     */
    public function getCopyRepository()
    {
        return $this->em->getRepository('MilexEmailBundle:Copy');
    }

    /**
     * @return \Milex\EmailBundle\Entity\StatDeviceRepository
     */
    public function getStatDeviceRepository()
    {
        return $this->em->getRepository('MilexEmailBundle:StatDevice');
    }
    ...
    public function getSentEmailToContactData($limit, \DateTime $dateFrom, \DateTime $dateTo, $options = [], $companyId = null, $campaignId = null, $segmentId = null)
    {
        ...
        foreach ($stats as $stat) {
            ...
            if (empty($stat['segment_id']) && !empty($stat['campaign_id'])) {
                ...
                $segmentMembership = $this->em->getRepository('MilexCampaignBundle:Campaign')
    ...
    public function getEmailListStats($email, $includeVariants = false, \DateTime $dateFrom = null, \DateTime $dateTo = null)
    {
        ...
        $chart     = new BarChart(
            [
                $this->translator->trans('milex.email.sent'),
                $this->translator->trans('milex.email.read'),
                $this->translator->trans('milex.email.failed'),
                $this->translator->trans('milex.email.clicked'),
                $this->translator->trans('milex.email.unsubscribed'),
                $this->translator->trans('milex.email.bounced'),
            ]
        );

        /** @var \Milex\EmailBundle\Entity\StatRepository $statRepo */
        $statRepo = $this->em->getRepository('MilexEmailBundle:Stat');

        /** @var \Milex\LeadBundle\Entity\DoNotContactRepository $dncRepo */
        $dncRepo = $this->em->getRepository('MilexLeadBundle:DoNotContact');

        /** @var \Milex\PageBundle\Entity\TrackableRepository $trackableRepo */
        $trackableRepo = $this->em->getRepository('MilexPageBundle:Trackable');
        ...
        if ($listCount) {
            ...
            if ($listCount > 1) {
                $chart->setDataset(
                    $this->translator->trans('milex.email.lists.combined'),
                    $combined,
                    0
                );
            } else {
    ...
    public function getEmailDeviceStats($email, $includeVariants = false, $dateFrom = null, $dateTo = null)
    {
        ...
        foreach ($results as $result) {
            if (empty($result['device'])) {
                $result['device'] = $this->translator->trans('milex.core.unknown');
            } else {
    ...
    public function sendEmailToLists(
        ...
        // @todo throw an exception in Milex 3 here.
    ...
    public function sendEmail(Email $email, $leads, $options = [])
    {
        ...
        /** @var \Milex\EmailBundle\Entity\EmailRepository $emailRepo */
        ...
        if (!$ignoreDNC) {
            ...
            if (!empty($dnc)) {
                foreach ($dnc as $removeMeId => $removeMeEmail) {
                    if ($dncAsError) {
                        $errors[$removeMeId] = $this->translator->trans('milex.email.dnc');
                    }
    ...
    public function sendEmailToUser(
        ...
        foreach ($to as $toAddress) {
            ...
            if (!$mailer->addTo($toAddress)) {
                $errors[] = "{$toAddress}: ".$this->translator->trans('milex.email.bounce.reason.bad_email');
                continue;
            }
        ...
        foreach ($users as $user) {
            ...
            if (!$mailer->setTo($user['email'], $user['firstname'].' '.$user['lastname'])) {
                $errors[] = "{$user['email']}: ".$this->translator->trans('milex.email.bounce.reason.bad_email');
                continue;
            }
    ...
    {
        /** @var \Milex\LeadBundle\Entity\LeadRepository $leadRepo */
        $leadRepo = $this->em->getRepository('MilexLeadBundle:Lead');
        $leadId   = (array) $leadRepo->getLeadByEmail($email, true);

        /** @var \Milex\LeadBundle\Entity\Lead[] $leads */
        $leads = [];
    ...
    public function setEmailDoNotContact($email, $reason = DoNotContact::BOUNCED, $comments = '', $flush = true, $leadId = null)
    {
        ...
        foreach ($leadId as $lead) {
            $dnc[] = $this->doNotContact->addDncForContact(
                $this->em->getReference('MilexLeadBundle:Lead', $lead),
                'email',
                $reason,
                $comments,
                $flush
    ...
    {
        $q->join('t', MILEX_TABLE_PREFIX.'emails', 'e', 'e.id = t.email_id')
    ...
    public function getBestHours($column, \DateTime $dateFrom, \DateTime $dateTo, array $filter = [], $canViewOthers = true, $timeFormat = 24
        ...
        $chart->setDataset($this->translator->trans('milex.widget.emails.best.hours.reads_total', ['%reads%'=>$total]), $counts);
    ...
    public function getIgnoredVsReadPieChartData($dateFrom, $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        $chart->setDataset($this->translator->trans('milex.email.graph.pie.ignored.read.failed.ignored'), ($sent - $read - $failed));
        $chart->setDataset($this->translator->trans('milex.email.graph.pie.ignored.read.failed.read'), $read);
        $chart->setDataset($this->translator->trans('milex.email.graph.pie.ignored.read.failed.failed'), $failed);
    ...
    public function getDeviceGranularityPieChartData($dateFrom, $dateTo)
    {
        ...
        if (empty($deviceStats)) {
            $deviceStats[] = [
                'count'   => 0,
                'device'  => $this->translator->trans('milex.report.report.noresults'),
                'list_id' => 0,
            ];
        }
        ...
        foreach ($deviceStats as $device) {
            $chart->setDataset(
                ($device['device']) ? $device['device'] : $this->translator->trans('milex.core.unknown'),
                $device['count']
            );
        }
    ...
    public function getEmailStatList($limit = 10, \DateTime $dateFrom = null, \DateTime $dateTo = null, $filters = [], $options = [])
    {
        ...
        $q->select('COUNT(DISTINCT t.id) AS count, e.id, e.name')
            ->from(MILEX_TABLE_PREFIX.'email_stats', 't')
            ->join('t', MILEX_TABLE_PREFIX.'emails', 'e', 'e.id = t.email_id')
            ->orderBy('count', 'DESC')
            ->groupBy('e.id')
            ->setMaxResults($limit);
    ...
    public function getEmailList($limit = 10, \DateTime $dateFrom = null, \DateTime $dateTo = null, $filters = [], $options = [])
    {
        ...
        $q->select('t.id, t.name, t.date_added, t.date_modified')
            ->from(MILEX_TABLE_PREFIX.'emails', 't')
            ->setMaxResults($limit);
    ...
    public function getUpcomingEmails($limit = 10, $canViewOthers = true)
    {
        ...
        $leadEventLogRepository = $this->em->getRepository('MilexCampaignBundle:LeadEventLog');
