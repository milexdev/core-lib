...
class AssetController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        //set limits
        $limit = $this->get('session')->get('milex.asset.limit', $this->get('milex.helper.core_parameters')->get('default_assetlimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $this->get('session')->get('milex.asset.filter', ''));
        $this->get('session')->set('milex.asset.filter', $search);
        ...
        $orderBy    = $this->get('session')->get('milex.asset.orderby', 'a.dateModified');
        $orderByDir = $this->get('session')->get('milex.asset.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $this->get('session')->set('milex.asset.asset', $lastPage);
            $returnUrl = $this->generateUrl('milex_asset_index', ['page' => $lastPage]);
        ...
        $this->get('session')->set('milex.asset.page', $page);
        ...
        return $this->delegateView([
            ...
            'passthroughVars' => [
                'activeLink'    => '#milex_asset_index',
                'milexContent' => 'asset',
                'route'         => $this->generateUrl('milex_asset_index', ['page' => $page]),
            ],
        ]);
    }
    ...
    public function viewAction($objectId)
    {
        ...
        $page = $this->get('session')->get('milex.asset.page', 1);

        $tmpl = $this->request->isXmlHttpRequest() ? $this->request->get('tmpl', 'details') : 'details';

        // Init the date range filter form
        $dateRangeValues = $this->request->get('daterange', []);
        $action          = $this->generateUrl('milex_asset_action', ['objectAction' => 'view', 'objectId' => $objectId]);
        $dateRangeForm   = $this->get('form.factory')->create(DateRangeType::class, $dateRangeValues, ['action' => $action]);

        if (null === $activeAsset) {
            //set the return URL
            $returnUrl = $this->generateUrl('milex_asset_index', ['page' => $page]);
            ...
            return $this->postActionRedirect([
                ...
                'flashes' => [
                    [
                        'type'    => 'error',
                        'msg'     => 'milex.asset.asset.error.notfound',
                        'msgVars' => ['%id%' => $objectId],
                    ],
                ],
            ]);
        } elseif (!$this->get('milex.security')->hasEntityAccess('asset:assets:viewown', 'asset:assets:viewother', $activeAsset->getCreatedBy())) {
            return $this->accessDenied();
        }
    ...
    public function previewAction($objectId)
    {
        ...
        if (null === $activeAsset || !$this->get('milex.security')->hasEntityAccess('asset:assets:viewown', 'asset:assets:viewother', $activeAsset->getCreatedBy())) {
        ...
        if ('1' === $download || '1' === $stream) {
            try {
                //set the uploadDir
                $activeAsset->setUploadDir($this->get('milex.helper.core_parameters')->get('upload_dir'));
    ...
    public function newAction($entity = null)
    {
        ...
        $entity->setMaxSize(FileHelper::convertMegabytesToBytes($this->get('milex.helper.core_parameters')->get('max_size')));

        $method  = $this->request->getMethod();
        $session = $this->get('session');

        if (!$this->get('milex.security')->isGranted('asset:assets:create')) {
            return $this->accessDenied();
        }

        $maxSize    = $model->getMaxUploadSize();
        $extensions = '.'.implode(', .', $this->get('milex.helper.core_parameters')->get('allowed_extensions'));

        $maxSizeError = $this->get('translator')->trans('milex.asset.asset.error.file.size', [
            '%fileSize%' => '{{filesize}}',
            '%maxSize%'  => '{{maxFilesize}}',
        ], 'validators');

        $extensionError = $this->get('translator')->trans('milex.asset.asset.error.file.extension.js', [
        ...
        $page   = $session->get('milex.asset.page', 1);
        $action = $this->generateUrl('milex_asset_action', ['objectAction' => 'new']);
        ...
        if ('POST' == $method) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    $entity->setUploadDir($this->get('milex.helper.core_parameters')->get('upload_dir'));
                    $entity->preUpload();
                    $entity->upload();
                    ...
                    $this->addFlash('milex.core.notice.created', [
                        ...
                        '%menu_link%' => 'milex_asset_index',
                        '%url%'       => $this->generateUrl('milex_asset_action', [
                            'objectAction' => 'edit',
                    ...
                    $returnUrl = $this->generateUrl('milex_asset_action', $viewParameters);
                    $template  = 'MauticAssetBundle:Asset:view';
                }
            } else {
                $viewParameters = ['page' => $page];
                $returnUrl      = $this->generateUrl('milex_asset_index', $viewParameters);
        ...
        return $this->delegateView([
            ...
            'passthroughVars' => [
                'activeLink'    => '#milex_asset_index',
                'milexContent' => 'asset',
                'route'         => $this->generateUrl('milex_asset_action', [
                    'objectAction' => 'new',
                ]),
            ],
        ]);
    ...
    public function editAction($objectId, $ignorePost = false)
    {
        ...
        //Create the form
        $action = $this->generateUrl('milex_asset_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form   = $model->createForm($entity, $this->get('form.factory'), $action);
        ...
        if (!$ignorePost && 'POST' == $method) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $returnUrl = $this->generateUrl('milex_asset_action', [
                        'objectAction' => 'view',
                        'objectId'     => $entity->getId(),
                    ]);
                ...
                $session->remove('milex.asestbuilder.'.$objectId.'.content');
                //unlock the entity
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'milex.asset.asset.notice.batch_deleted',
                    'msgVars' => [
    ...
    public function remoteAction()
    {
        ...
        return $this->delegateView([
            ...
            'passthroughVars' => [
                'activeLink'    => '#milex_asset_index',
                'milexContent' => 'asset',
                'route'         => $this->generateUrl('milex_asset_index', ['page' => $this->get('session')->get('milex.asset.page', 1)]),
            ],
        ]);
    }
