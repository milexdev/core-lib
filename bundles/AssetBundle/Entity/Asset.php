    ...
    public static function validateFile($object, ExecutionContextInterface $context)
    {
        if ($object->isLocal()) {
            ...
            if ($object->isNew() && null === $tempName && null === $path) {
                $context->buildViolation('milex.asset.asset.error.missing.file')
                    ->atPath('tempName')
            ...
            if (null === $object->getTitle()) {
                $context->buildViolation('milex.asset.asset.error.missing.title')
                    ->atPath('title')
                    ->setTranslationDomain('validators')
                    ->addViolation();
            }
            ...
            if (null === $object->getRemotePath()) {
                $context->buildViolation('milex.asset.asset.error.missing.remote.path')
                    ->atPath('remotePath')
                    ->setTranslationDomain('validators')
                    ->addViolation();
            }
