...
class SendChannelBroadcastCommand extends ModeratedCommand
{
    ...
    {
        $this->setName('milex:broadcasts:send')
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $translator->setLocale($this->getContainer()->get('milex.helper.core_parameters')->get('locale'));
        ...
        $table
            ->setHeaders([$translator->trans('milex.core.channel'), $translator->trans('milex.core.channel.broadcast_success_count'), $translator->trans('milex.core.channel.broadcast_failed_count')])
            ->setRows($rows);
        $table->render();
