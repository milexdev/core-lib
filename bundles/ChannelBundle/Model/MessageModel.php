    ...
    public function getChannels()
    {
        if (!self::$channels) {
            ...
            foreach ($channels as $channel => $config) {
                ...
                switch (true) {
                    case $this->translator->hasId('milex.channel.'.$channel):
                        $label = $this->translator->trans('milex.channel.'.$channel);
                        break;
                    case $this->translator->hasId('milex.'.$channel):
                        $label = $this->translator->trans('milex.'.$channel);
                        break;
                    case $this->translator->hasId('milex.'.$channel.'.'.$channel):
                        $label = $this->translator->trans('milex.'.$channel.'.'.$channel);
                        break;
