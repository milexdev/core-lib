...
class MessageController extends AbstractStandardFormController
{
    ...
    protected function getViewArguments(array $args, $action)
    {
        ...
        switch ($action) {
            case 'index':
                $viewParameters = [
                    'headerTitle' => $this->get('translator')->trans('milex.channel.messages'),
                    'listHeaders' => [
                        [
                            'text'  => 'milex.core.channels',
            ...
            case 'view':
                ...
                $chart->setDataset(
                    $this->get('translator')->trans('milex.core.all'),
                ...
                $messagedLeads = [
                    'all' => $this->forward(
                        ...
                        [
                            'objectId'   => $message->getId(),
                            'page'       => $this->get('session')->get('milex.'.$this->getSessionBase('all').'.contact.page', 1),
                            'ignoreAjax' => true,
                            'channel'    => 'all',
                        ]
                    )->getContent(),
                        ...
                        $messagedLeads[$channel->getChannel()] = $this->forward(
                            ...
                            [
                                ...
                                'page'     => $this->get('session')->get(
                                    'milex.'.$this->getSessionBase($channel->getChannel()).'.contact.page',
                                    1
                                ),
                                'ignoreAjax' => true,
