...
class BuildJsSubscriber implements EventSubscriberInterface
{
    ...
    {
        $pageTrackingUrl = $this->router->generate('milex_page_tracker', [], UrlGeneratorInterface::ABSOLUTE_URL);
        ...
        $pageTrackingCORSUrl = str_replace(
            ['http://', 'https://'],
            '',
            $this->router->generate('milex_page_tracker_cors', [], UrlGeneratorInterface::ABSOLUTE_URL)
        );
        $contactIdUrl = str_replace(
            ['http://', 'https://'],
            '',
            $this->router->generate('milex_page_tracker_getcontact', [], UrlGeneratorInterface::ABSOLUTE_URL)
        );
...
(function(m, l, n, d) {
    ...
    m.deliverPageEvent = function(event, params) {
        ...
        MilexJS.makeCORSRequest('POST', m.pageTrackingCORSUrl, params, 
        function(response) {
            MilexJS.dispatchEvent('milexPageEventDelivered', {'event': event, 'params': params, 'response': response});
        },
        function() {
    ...
    m.buildTrackingImage = function(pageview, params) {
        ...
        m.trackingPixel.onload = function(e) {
            MilexJS.dispatchEvent('milexPageEventDelivered', {'event': pageview, 'params': params, 'image': true});
        };
    ...
    m.sendPageview = function(pageview) {
        ...
        if (queue) {
            for (var i=0; i<queue.length; i++) {
                ...
                params = MilexJS.appendTrackedContact(params);
    ...
    document.addEventListener('eventAddedToMilexQueue', function(e) {
      if (MilexJS.ensureEventContext(e, 'send', 'pageview')) {
          m.sendPageview(e.detail);
      }
    });
})(MilexJS, location, navigator, document);
JS;

        $event->appendJs($js, 'Milex Tracking Pixel');
    }
    ...
    public function onBuildJsForVideo(BuildJsEvent $event)
    {
        ...
        $milexBaseUrl   = $this->router->generate('milex_base_index', [], UrlGeneratorInterface::ABSOLUTE_URL);
        ...
        $milexBaseUrl   = str_replace('/index_dev.php', '', $milexBaseUrl);
...
JS;
        ...
        $js = <<<JS
MilexJS.initGatedVideo = function () {
    MilexJS.videoElements = MilexJS.videoElements || document.getElementsByTagName('video');
 
    if (! MilexJS.videoElements.length) {
        MilexJS.videoElements = null;
        return;
    }

    MilexJS.milexInsertedScripts = MilexJS.milexInsertedScripts || {};
    
    if ("undefined" == typeof jQuery && "undefined" == typeof MilexJS.milexInsertedScripts.jQuery) {
        MilexJS.insertScript('{$jQueryUrl}');
        MilexJS.milexInsertedScripts.jQuery = true;
    }

    if ("undefined" == typeof Froogaloop && "undefined" == typeof MilexJS.milexInsertedScripts.Froogaloop) {
        MilexJS.insertScript('https://f.vimeocdn.com/js/froogaloop2.min.js');
        MilexJS.milexInsertedScripts.Froogaloop = true;
    }
    
    if ("undefined" == typeof jQuery || "undefined" == typeof Froogaloop) {
        MilexJS.gatedVideoTimeoutID = window.setTimeout(MilexJS.initGatedVideo, 1000);
    } else {
        MilexJS.insertStyle('{$mediaElementCss}');

        {$mediaElementJs}
        
        MilexJS.processGatedVideos(MilexJS.videoElements);
    }
};

MilexJS.processGatedVideos = function (videoElements) {
    window.mediaPlayers  = [];

    window.setInterval(function (e) {
        MilexJS.iterateCollection(mediaPlayers)(function(node, i){
            if (mediaPlayers[i].views) {
                MilexJS.iterateCollection(mediaPlayers[i].views)(function(nodeA, iA){
                    if (!node.views[iA].sent) {
                        MilexJS.makeCORSRequest('POST', '{$milexBaseUrl}video/hit', node.views[iA], function (data) {
    ...
    MilexJS.addVideoView = function (i, totalWatched) {
        if (totalWatched == 0) {
            return;
        }
        ...
        MilexJS.iterateCollection(mediaPlayers[i].views)(function(node, j){
    ...
    MilexJS.endCurrentView = function (i) {
        var viewId = 0;
    ...
    MilexJS.iterateCollection(videoElements)(function(node, i){
        // If we don't have a formId, and it's not explicitly set as a Milex controlled video, return.
        if (!node.dataset.formId && !node.dataset.milexVideo) {
            return;
        }
        ...
        if (node.dataset.formId) {
            var cookieName = 'milex-player-'+i+'-'+node.dataset.formId;
            MilexJS.makeCORSRequest('GET', '{$milexBaseUrl}form/embed/' + node.dataset.formId, {}, function (data) {
        ...
        mediaPlayers[i].player = new MediaElementPlayer('#' + node.id, {features: playerFeatures, alwaysShowControls: true, enableKeyboard: false, success: function (mediaElement, domElement) {
            ...
            if (iframes) {
                MilexJS.iterateCollection(iframes)(function(node, i) {
                    node.setAttribute('tabindex', -1);
                });
            }

            mediaElement.addEventListener('ended', function(e) {
                MilexJS.endCurrentView(i);
            });
            ...
            mediaElement.addEventListener('timeupdate', function (e) {
                ...
                MilexJS.addVideoView(i, currentTime);
                ...
                if (node.dataset.formId && document.cookie.indexOf(cookieName) == -1 && currentTime >= node.dataset.gateTime && mediaPlayers[i].inPoster === false && mediaPlayers[i].success === false) {
                    ...
                    mediaPlayers[i].form.className += ' milex-gated-video-style';
                    
                    mediaPlayers[i].form.addEventListener('submit', function (e) {
                        e.preventDefault();
                        
                        MilexJS.makeCORSRequest(
                            'POST',
                            ...
                            function (data) {
                                ...
                                if (data.message) {
                                    ...
                                    mediaPlayers[i].poster.innerHTML = '<p style="padding:20px;padding-top:' + position + 'px;" class="milex-response">' + data.message + '</p>';
                                    setTimeout(function(){
                                        mediaPlayers[i].poster.style.display = 'none';
                                        mediaPlayers[i].player.play();
                                    ...
                                    jQuery('#milexform_'+data.formName+' .milexform-errormsg').css('display', 'none');
                                    // Display validation errors
                                    jQuery.each(data.validationErrors, function (field, message) {
                                        if (jQuery('#milexform_'+data.formName+'_'+field+' .milexform-errormsg')) {
                                            jQuery('#milexform_'+data.formName+'_'+field+' .milexform-errormsg').css('display', 'block');
                                        }
                                    });
                                }
                            }
...
MilexJS.documentReady(MilexJS.initGatedVideo);
JS;
        $event->appendJs($js, 'Milex Gated Videos');
    }
...
JS;
        ...
        $js .= <<<'JS'
        MilexJS.mtcEventSet=false;
        document.addEventListener('milexPageEventDelivered', function(e) {
            var detail   = e.detail;
            if (!MilexJS.mtcEventSet && detail.response && detail.response.events) {
                MilexJS.setTrackedEvents(detail.response.events);
            }
      });
      
      
MilexJS.setTrackedEvents = function(events) {
        MilexJS.mtcEventSet=true;
...
JS;
        $event->appendJs($js, 'Milex 3rd party tracking pixels');
    }
}
