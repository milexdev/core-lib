...
class TrackableModelTest extends TestCase
{
     ...
     *
     * @covers \Milex\PageBundle\Model\TrackableModel::extractTrackablesFromHtml
     * @covers \Milex\PageBundle\Model\TrackableModel::parseContentForTrackables
     * @covers \Milex\PageBundle\Model\TrackableModel::createTrackingTokens
     * @covers \Milex\PageBundle\Model\TrackableModel::prepareUrlForTracking
     * @covers \Milex\PageBundle\Model\TrackableModel::prepareContentWithTrackableTokens
     */
     ...
     *
     * @covers \Milex\PageBundle\Model\TrackableModel::extractTrackablesFromText
     * @covers \Milex\PageBundle\Model\TrackableModel::parseContentForTrackables
     * @covers \Milex\PageBundle\Model\TrackableModel::createTrackingTokens
     * @covers \Milex\PageBundle\Model\TrackableModel::prepareUrlForTracking
     * @covers \Milex\PageBundle\Model\TrackableModel::prepareContentWithTrackableTokens
     ...
     *
     * @covers \Milex\PageBundle\Model\TrackableModel::validateTokenIsTrackable
     * @covers \Milex\PageBundle\Model\TrackableModel::parseContentForTrackables
     * @covers \Milex\PageBundle\Model\TrackableModel::prepareUrlForTracking
     */
    ...
    public function testTokenizedHostWithScheme()
    {
        ...
        list($content, $trackables) = $model->parseContentForTrackables(
            $this->generateContent($url, 'html'),
            [
                '{contactfield=foo}' => 'https://milex.org',
            ],
            'email',
            1
        );
    ...
    {
        $url          = '{contactfield=website|https://milex.org}';
        $model        = $this->getModel();
        ...
        list($content, $trackables) = $model->parseContentForTrackables(
            $inputContent,
            [
                '{contactfield=website}' => 'https://milex.org/about-us',
            ],
            'email',
            1
        );
        ...
        $this->assertEquals('{contactfield=website|https://milex.org}', $trackables[$trackableKey]->getRedirect()->getUrl());
    }
    ...
    public function testCssUrlsAreNotConvertedIfThereAreNoLinks()
    {
        ...
        list($content, $trackables) = $model->parseContentForTrackables(
            '<style> .mf-modal { background-image: url(\'https://www.milex.org/wp-content/uploads/2014/08/iTunesArtwork.png\'); } </style>',
            [],
            'email',
            1
        );
...
TEXT;
    ...
    protected function generateContent($urls, $type, $doNotTrack = false)
    {
        ...
        foreach ($urls as $url) {
            if ('html' == $type) {
                $dnc = ($doNotTrack) ? ' milex:disable-tracking' : '';
