...
use Milex\CoreBundle\Exception\InvalidDecodedStringException;
use Milex\CoreBundle\Helper\TrackingPixelHelper;
use Milex\CoreBundle\Helper\UrlHelper;
use Milex\LeadBundle\Helper\PrimaryCompanyHelper;
...
use Milex\PageBundle\Event\TrackingEvent;
use Milex\PageBundle\Helper\TrackingHelper;
use Milex\PageBundle\Model\PageModel;
use Milex\PageBundle\Model\Tracking404Model;
use Milex\PageBundle\Model\VideoModel;
...
class PublicController extends CommonFormController
{
    ...
    public function indexAction($slug, Request $request)
    {
        ...
        if (!empty($entity) && !$entity->getIsPreferenceCenter()) {
            ...
            if (!$userAccess) {
                ...
                if (count($childrenVariants)) {
                    ...
                    if (count($variants)) {
                        ...
                        $variantCookie = $this->request->cookies->get('milex_page_'.$entity->getId());
                        ...
                        if (!empty($variantCookie)) {
                            ...
                            $this->get('milex.helper.cookie')->setCookie(
                                'milex_page_'.$entity->getId(),
                                $useId,
                                3600 * 24 * 14
                            );
            ...
            // Generate contents
            $analytics = $this->get('milex.helper.template.analytics')->getCode();

            $BCcontent = $entity->getContent();
            $content   = $entity->getCustomHtml();
            // This condition remains so the Milex v1 themes would display the content
            ...
            $this->get('templating.helper.assets')->addScript(
                $this->get('router')->generate('milex_js', [], UrlGeneratorInterface::ABSOLUTE_URL),
        ...
        $tracking404Model = $this->get('milex.page.model.tracking.404');
    ...
    public function previewAction($id)
    {
        ...
        if (empty($content) && !empty($BCcontent)) {
            ...
            $content = str_replace('</head>', $analytics.$this->renderView('MilexPageBundle:Page:preview_header.html.php')."\n</head>", $content);
        }
    ...
    public function trackingAction(Request $request)
    {
        ...
        if (!$this->get('milex.security')->isAnonymous()) {
        ...
        $trackedDevice = $this->get('milex.lead.service.device_tracking_service')->getTrackedDevice();
        $trackingId    = (null === $trackedDevice ? null : $trackedDevice->getTrackingId());

        /** @var TrackingHelper $trackingHelper */
        $trackingHelper = $this->get('milex.page.helper.tracking');
    ...
    {
        $logger = $this->container->get('monolog.logger.milex');
        ...
        if (null === $redirect || !$redirect->isPublished(false)) {
            ...
            throw $this->createNotFoundException($this->translator->trans('milex.core.url.error.404', ['%url%' => $url]));
        }
        ...
        $ipAddress = $this->container->get('milex.helper.ip_lookup')->getIpAddress();
        ...
        if (isset($ct)) {
            if ($ipAddress->isTrackable()) {
                ...
                $primaryCompanyHelper = $this->get('milex.lead.helper.primary_company');
                $leadArray            = ($lead) ? $primaryCompanyHelper->getProfileFieldsWithPrimaryCompany($lead) : [];

                $url = TokenHelper::findLeadTokens($url, $leadArray, true);
            }

            if (false !== strpos($url, $this->generateUrl('milex_asset_download'))) {
                $url .= '?ct='.$ct;
            }
        }
            ...
            if (isset($slotConfig['type']) && 'slideshow' == $slotConfig['type']) {
                ...
                if (empty($options['slides'])) {
                    $options['slides'] = [
                        [
                            'order'            => 0,
                            'background-image' => $assetsHelper->getUrl('media/images/milex_logo_lb200.png'),
                            'captionheader'    => 'Caption 1',
                        ],
                        [
                            'order'            => 1,
                            'background-image' => $assetsHelper->getUrl('media/images/milex_logo_db200.png'),
                ...
                $renderingEngine = $this->container->get('templating')->getEngine('MilexPageBundle:Page:Slots/slideshow.html.php');
                $slotsHelper->set($slot, $renderingEngine->render('MilexPageBundle:Page:Slots/slideshow.html.php', $options));
