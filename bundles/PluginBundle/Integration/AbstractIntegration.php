...
use Milex\CoreBundle\Entity\CommonEntity;
use Milex\CoreBundle\Entity\FormEntity;
...
use Milex\PluginBundle\Event\PluginIntegrationAuthCallbackUrlEvent;
use Milex\PluginBundle\Event\PluginIntegrationFormBuildEvent;
use Milex\PluginBundle\Event\PluginIntegrationFormDisplayEvent;
use Milex\PluginBundle\Event\PluginIntegrationKeyEvent;
use Milex\PluginBundle\Event\PluginIntegrationRequestEvent;
use Milex\PluginBundle\Exception\ApiErrorException;
use Milex\PluginBundle\Helper\Cleaner;
use Milex\PluginBundle\Helper\oAuthHelper;
 ...
 *
 * @deprecated To be removed in Milex 6.0. Please use the IntegrationsBundle instead, which is meant to be a drop-in replacement for AbstractIntegration.
 */
abstract class AbstractIntegration implements UnifiedIntegrationInterface
{
    ...
    public function __construct(
        ...
        $this->session                = (!defined('IN_MILEX_CONSOLE')) ? $session : null;
        $this->request                = (!defined('IN_MILEX_CONSOLE')) ? $requestStack->getCurrentRequest() : null;
        $this->router                 = $router;
    ...
    /**
     * @return \Milex\CoreBundle\Translation\Translator
     */
     ...
     *
     * Example:
     *  'cloud_storage' => 'milex.integration.form.features.cloud_storage.tooltip'
     *
     * @return array
     */
    ...
    {
        return 'MilexPluginBundle:Integration:form.html.php';
    }
    ...
    {
        return 'MilexPluginBundle:FormTheme\Integration';
    }
    ...
    public function getRequiredKeyFields()
    {
        switch ($this->getAuthenticationType()) {
            case 'oauth1a':
                return [
                    'consumer_id'     => 'milex.integration.keyfield.consumerid',
                    'consumer_secret' => 'milex.integration.keyfield.consumersecret',
                ];
            case 'oauth2':
                return [
            ...
            case 'key':
                return [
                    'key' => 'milex.integration.keyfield.api',
                ];
            case 'basic':
                return [
                    'username' => 'milex.integration.keyfield.username',
                    'password' => 'milex.integration.keyfield.password',
                ];
            default:
                return [];
        }
    ...
    public function createIntegrationEntity(
        ...
        $date = (defined('MILEX_DATE_MODIFIED_OVERRIDE')) ? \DateTime::createFromFormat('U', MILEX_DATE_MODIFIED_OVERRIDE)
            : new \DateTime();
        ...
        if ($persist) {
            $this->em->getRepository('MilexPluginBundle:IntegrationEntity')->saveEntity($entity);
        }
    ...
    {
        return $this->em->getRepository('MilexPluginBundle:IntegrationEntity');
    }
            ...
            case 'oauth2':
                if ($this->session) {
                    ...
                    if ($state && $state !== $givenState) {
                        ...
                        throw new ApiErrorException($this->translator->trans('milex.integration.auth.invalid.state'));
                    }
                }
        ...
        if (is_array($data) && isset($data[$authTokenKey])) {
            ...
            if (empty($error)) {
                $error = $this->translator->trans(
                    'milex.integration.error.genericerror',
                    [],
                    'flashes'
                );
            }
     ...
     * @return array
     */
    public function cleanUpFields(Integration $entity, array $milexLeadFields, array $milexCompanyFields)
    {
        ...
        $milexLeadFields['milexContactId']                   = '';
        $milexLeadFields['milexContactTimelineLink']         = '';
        $milexLeadFields['milexContactIsContactableByEmail'] = '';
         ...
         * @param $fieldType
         */
        $cleanup = function (&$mappedFields, $integrationFields, $milexFields, $fieldType) use (&$missingRequiredFields, &$featureSettings) {
            $updateKey    = ('companyFields' === $fieldType) ? 'update_milex_company' : 'update_milex';
            $removeFields = array_keys(array_diff_key($mappedFields, $integrationFields));

            // Find all the mapped fields that no longer exist in Milex
            if ($nonExistentFields = array_diff($mappedFields, array_keys($milexFields))) {
                // Remove those fields
            ...
            foreach ($mappedFields as $field => $milexField) {
                if (!isset($featureSettings[$updateKey][$field])) {
                    // Assume it's mapped to Milex
        ...
        if ($submittedObjects) {
            if (in_array('company', $submittedObjects)) {
                ...
                if (isset($availableIntegrationFields['company'])) {
                    $cleanup($submittedCompanyFields, $availableIntegrationFields['company'], $milexCompanyFields, 'companyFields');
        ...
        if (!empty($leadFields)) {
            $cleanup($submittedFields, $leadFields, $milexLeadFields, 'leadFields');
    ...
    public function populateLeadData($lead, $config = [])
    {
        ...
        foreach ($availableFields as $key => $field) {
            ...
            if (isset($leadFields[$integrationKey])) {
                if ('milexContactTimelineLink' === $leadFields[$integrationKey]) {
                    $matched[$integrationKey] = $this->getContactTimelineLink($leadId);

                    continue;
                }
                if ('milexContactIsContactableByEmail' === $leadFields[$integrationKey]) {
                    $matched[$integrationKey] = $this->getLeadDoNotContact($leadId);

                    continue;
                }
                if ('milexContactId' === $leadFields[$integrationKey]) {
                    $matched[$integrationKey] = $lead->getId();
                    continue;
                }
                $milexKey = $leadFields[$integrationKey];
                if (isset($fields[$milexKey]) && '' !== $fields[$milexKey] && null !== $fields[$milexKey]) {
    ...
    public function populateCompanyData($entity, $config = [])
    {
        ...
        foreach ($availableFields as $key => $field) {
            ...
            if (isset($companyFields[$key])) {
                $milexKey = $companyFields[$key];
                if (isset($fields[$milexKey]) && !empty($fields[$milexKey])) {
                    $matched[$integrationKey] = $this->cleanPushData($fields[$milexKey], (isset($field['type'])) ? $field['type'] : 'string');
                }
            }
    ...
    /**
     * Takes profile data from an integration and maps it to Milex's lead fields.
     *
     * @param       $data
     * @param array $config
     * @param null  $object
     ...
     * @return array
     */
    public function populateMilexLeadData($data, $config = [], $object = null)
    {
        ...
        foreach ($gleanedData as $key => $field) {
            if (isset($fields[$key]) && isset($gleanedData[$key])
                && $this->translator->trans('milex.integration.form.lead.unknown') !== $gleanedData[$key]
            ) {
    ...
    /**
     * Create or update existing Milex lead from the integration's profile data.
     *
     ...
     * @return Lead
     */
    public function getMilexLead($data, $persist = true, $socialCache = null, $identifiers = null)
    {
        if (is_object($data)) {
        ...
        $matchedFields = $this->populateMilexLeadData($data);
        ...
        if (count($uniqueLeadFieldData)) {
            $existingLeads = $this->em->getRepository('MilexLeadBundle:Lead')
    ...
    /**
     * @return \Milex\CoreBundle\Model\NotificationModel
     */
    ...
    public function logIntegrationError(\Exception $e, Lead $contact = null)
    {
        ...
        if ($e instanceof ApiErrorException) {
            if (null === $this->adminUsers) {
                $this->adminUsers = $this->em->getRepository('MilexUserBundle:User')->getEntities(
                    [
                        'filter' => [
                            'force' => [
                                [
            ...
            if ($contact || $contact = $e->getContact()) {
                ...
                $contactName = $this->getTranslator()->trans('milex.integration.error.generic_contact_name', ['%id%' => $contactId]);
            }
        ...
        $logger->addError('INTEGRATION ERROR: '.$this->getName().' - '.(('dev' == MILEX_ENV) ? (string) $e : $e->getMessage()));
    }
    ...
    public function getFormNotes($section)
    {
        if ('leadfield_match' == $section) {
            return ['milex.integration.form.field_match_notes', 'info'];
        } else {
            return ['', 'info'];
        }
    }
    ...
    /**
     * @param array $mapping           array of [$milexId => ['entity' => FormEntity, 'integration_entity_id' => $integrationId]]
     * @param       $integrationEntity
     * @param       $internalEntity
     * @param array $params
     */
    ...
    protected function getLastSyncDate($entity = null, $params = [], $ignoreEntityChanges = true)
    {
        ...
        return (defined('MILEX_DATE_MODIFIED_OVERRIDE')) ? \DateTime::createFromFormat('U', MILEX_DATE_MODIFIED_OVERRIDE)
            : new \DateTime();
    }
    ...
    /**
     * Get pseudo fields from milex, these are lead properties we want to map to integration fields.
     *
     * @param $lead
     *
     * @return mixed
     */
    public function getCompoundMilexFields($lead)
    {
        if ($lead['internal_entity_id']) {
            $lead['milexContactId']                   = $lead['internal_entity_id'];
            $lead['milexContactTimelineLink']         = $this->getContactTimelineLink($lead['internal_entity_id']);
            $lead['milexContactIsContactableByEmail'] = $this->getLeadDoNotContact($lead['internal_entity_id']);
        }
     ...
     * @return bool
     */
    public function isCompoundMilexField($fieldName)
    {
        $compoundFields = [
            'milexContactTimelineLink' => 'milexContactTimelineLink',
            'milexContactId'           => 'milexContactId',
        ];

        if (true === $this->updateDncByDate()) {
            $compoundFields['milexContactIsContactableByEmail'] = 'milexContactIsContactableByEmail';
        }
    ...
    /**
     * Because so many integrations extend this class and milex.http.client is not in the
