 ...
 * @param data
 */
Milex.loadContent = function (route, link, method, target, showPageLoading, callback, data) {
    if (typeof Milex.loadContentXhr == 'undefined') {
        Milex.loadContentXhr = {};
    } else if (typeof Milex.loadContentXhr[target] != 'undefined') {
        Milex.loadContentXhr[target].abort();
    }

    showPageLoading = (typeof showPageLoading == 'undefined' || showPageLoading) ? true : false;

    Milex.loadContentXhr[target] = mQuery.ajax({
        ...
        success: function (response) {
            if (response) {
                ...
                if (response.redirect) {
                    Milex.redirectWithBackdrop(response.redirect);
        ...
        complete: function () {
            if (typeof callback !== 'undefined') {
                if (typeof callback == 'function') {
                    ...
                    window["Milex"][callback].apply('window', []);
                }
            }
            Milex.generatePageTitle( route );
            delete Milex.loadContentXhr[target];
        }
    });
 ...
 * @param callback
 */
Milex.loadAjaxColumn = function(elementName, route, callback){
    ...
    if (mQuery(className).length) {
        ...
        while (ids.length > 0) {
            ...
            Milex.ajaxActionRequest(
                route,
                {ids: batchIds, entityId: Milex.getEntityId()},
                function (response) {
 ...
 * @param sortElement
 */
Milex.sortTableByColumn = function(tableId, sortElement, removeZero){
 ...
 * @param callback
 */
Milex.getCallback = function (callback) {
 ...
 * @param route
 */
Milex.generatePageTitle = function(route){
    ...
    if (-1 !== route.indexOf('timeline')) {
        ...
        mQuery('title').html( currentModule[0].toUpperCase() + currentModule.slice(1) + ' | ' + currentModuleItem + ' | Milex' );
    } else {
        //loading basic title
        mQuery('title').html( mQuery('.page-header h3').html() + ' | Milex' );
    }
};
 ...
 * @param response
 */
Milex.processPageContent = function (response) {
    if (response) {
        Milex.deactivateBackgroup();

        if (response.errors && 'dev' == milexEnv) {
        ...
        Milex.onPageUnload(response.target, response);
        ...
        if (response.route) {
            ...
            //update Title
            Milex.generatePageTitle( response.route );
        }

        if (response.target == '#app-content') {
            //update type of content displayed
            if (response.milexContent) {
                milexContent = response.milexContent;
            }
        ...
        Milex.onPageLoad(response.target, response);
    }
};
 ...
 */
Milex.onPageLoad = function (container, response, inModal) {
    Milex.initDateRangePicker(container + ' #daterange_date_from', container + ' #daterange_date_to');

    //initiate links
    Milex.makeLinksAlive(mQuery(container + " a[data-toggle='ajax']"));

    //initialize forms
    mQuery(container + " form[data-toggle='ajax']").each(function (index) {
        Milex.ajaxifyForm(mQuery(this).attr('name'));
    });

    //initialize ajax'd modals
    Milex.makeModalsAlive(mQuery(container + " *[data-toggle='ajaxmodal']"))

    //initialize embedded modal forms
    Milex.activateModalEmbeddedForms(container);

    //initalize live search boxes
    mQuery(container + " *[data-toggle='livesearch']").each(function (index) {
        Milex.activateLiveSearch(mQuery(this), "lastSearchStr", "liveCache");
    });

    //initialize list filters
    mQuery(container + " *[data-toggle='listfilter']").each(function (index) {
        Milex.activateListFilterSelect(mQuery(this));
    });
    ...
    mQuery(container + " div.sortable-panels").each(function () {
        Milex.activateSortablePanels(this);
    });
    ...
    mQuery(container + " a[data-toggle='download']").on('click.download', function (event) {
        ...
        Milex.initiateFileDownload(mQuery(this).attr('href'));
    });

    Milex.makeConfirmationsAlive(mQuery(container + " a[data-toggle='confirmation']"));

    //initialize date/time
    mQuery(container + " *[data-toggle='datetime']").each(function() {
        Milex.activateDateTimeInputs(this, 'datetime');
    });

    mQuery(container + " *[data-toggle='date']").each(function() {
        Milex.activateDateTimeInputs(this, 'date');
    });

    mQuery(container + " *[data-toggle='time']").each(function() {
        Milex.activateDateTimeInputs(this, 'time');
    });

    // Initialize callback options
    mQuery(container + " *[data-onload-callback]").each(function() {
        var callback = function(el) {
            if (typeof window["Milex"][mQuery(el).attr('data-onload-callback')] == 'function') {
                window["Milex"][mQuery(el).attr('data-onload-callback')].apply('window', [el]);
            }
        }
    ...
    mQuery(container + " input[data-toggle='color']").each(function() {
        Milex.activateColorPicker(this);
    });

    mQuery(container + " select").not('.multiselect, .not-chosen').each(function() {
        Milex.activateChosenSelect(this);
    });

    mQuery(container + " select.multiselect").each(function() {
        Milex.activateMultiSelect(this);
    });

    Milex.activateLookupTypeahead(mQuery(container));
    ...
    mQuery(container + " .nav-overflow-tabs ul").each(function() {
        Milex.activateOverflowTabs(this);
    });

    mQuery(container + " .nav.sortable").each(function() {
        Milex.activateSortableTabs(this);
    });

    // Initialize tab delete buttons
    Milex.activateTabDeleteButtons(container);
    ...
    mQuery(container + ' input[class=list-checkbox]').on('change', function () {
        var disabled = Milex.batchActionPrecheck(container) ? false : true;
    ...
    Milex.activateGlobalFroalaOptions();
    Milex.getBuilderContainer = function() {
        return container;
    }

    // This turns all textarea elements with class "editor" into CKEditor ones, except for Dynamic Content elements, which can be initialized with Milex.setDynamicContentEditors().
    if (mQuery(container + ' textarea.editor:not(".editor-dynamic-content")').length && Milex.getActiveBuilderName() === 'legacy') {
        ...
        mQuery(container + ' textarea.editor:not(".editor-dynamic-content")').each(function () {
            ...
            Milex.ConvertFieldToCkeditor(textarea, ckEditorToolbar);
        });
    }
    ...
    if (response && response.updateSelect && typeof response.id !== 'undefined') {
        ...
        Milex.updateEntitySelect(response);
    }

    //run specific on loads
    var contentSpecific = false;
    if (response && response.milexContent) {
        contentSpecific = response.milexContent;
    } else if (container == 'body') {
        contentSpecific = milexContent;
    }
    ...
    if (container == '#app-content' || container == 'body') {
        ...
        Milex.bindGlobalKeyboardShortcuts();
    ...
    if (contentSpecific && typeof Milex[contentSpecific + "OnLoad"] == 'function') {
        if (inModal || typeof Milex.loadedContent[contentSpecific] == 'undefined') {
            Milex.loadedContent[contentSpecific] = true;
            Milex[contentSpecific + "OnLoad"](container, response);
        }
    }
    ...
    if (!inModal && container == 'body') {
        ...
        if (mQuery('#globalSearchContainer').length) {
            mQuery('#globalSearchContainer .search-button').click(function () {
                ...
                mQuery('body').on('click.globalsearch', function (event) {
                    ...
                    if (!mQuery(target).parents('#globalSearchContainer').length && !mQuery(target).parents('#globalSearchDropdown').length) {
                        Milex.closeGlobalSearchResults();
                    }
                });
            });
            ...
            Milex.activateLiveSearch("#globalSearchInput", "lastGlobalSearchStr", "globalLivecache");
        }
    }

    Milex.renderCharts(container);
    Milex.renderMaps(container);
    Milex.stopIconSpinPostEvent();
...
Milex.setDynamicContentEditors = function(container) {
    ...
    if (mQuery(container + ' textarea.editor-dynamic-content').length && Milex.getActiveBuilderName() === 'legacy') {
 ...
 * @param jQueryObject
 */
Milex.activateLookupTypeahead = function(containerEl) {
 ...
 * @param jQueryObject
 */
Milex.makeConfirmationsAlive = function(jQueryObject) {
    jQueryObject.off('click.confirmation');
    jQueryObject.on('click.confirmation', function (event) {
        event.preventDefault();
        MilexVars.ignoreIconSpin = true;
        return Milex.showConfirmation(this);
    });
};
 ...
 * @param jQueryObject
 */
Milex.makeModalsAlive = function(jQueryObject) {
 ...
 * @param jQueryObject
 */
Milex.makeLinksAlive = function(jQueryObject) {
 ...
 */
Milex.onPageUnload = function (container, response) {
    ...
    if (typeof container != 'undefined') {
        ...
        if (typeof MilexVars.modalsReset == 'undefined') {
            MilexVars.modalsReset = {};
        }
    ...
    if (container == '#app-content') {
        ...
        if (typeof Milex.chartObjects !== 'undefined') {
            mQuery.each(Milex.chartObjects, function (i, chart) {
                chart.destroy();
            });
            Milex.chartObjects = [];
        }
        ...
        if (typeof Milex.mapObjects !== 'undefined') {
            mQuery.each(Milex.mapObjects, function (i, map) {
                Milex.destroyMap(map);
            });
            Milex.mapObjects = [];
        }

        // trash tokens to save some memory
        if (typeof Milex.builderTokens !== 'undefined') {
            Milex.builderTokens = {};
        }
    } else if (response && response.milexContent) {
        contentSpecific = response.milexContent;
    }

    if (contentSpecific) {
        if (typeof Milex[contentSpecific + "OnUnload"] == 'function') {
            Milex[contentSpecific + "OnUnload"](container, response);
        }

        if (typeof Milex.loadedContent[contentSpecific] !== 'undefined') {
            delete Milex.loadedContent[contentSpecific];
        }
    }
};
 ...
 * @returns {boolean}
 */
Milex.ajaxifyLink = function (el, event) {
    if (mQuery(el).hasClass('disabled')) {
        return false;
    }

    var route = mQuery(el).attr('href');
    if (route.indexOf('javascript') >= 0 || MilexVars.routeInProgress === route) {
        return false;
    }

    if (route.indexOf('batchExport') >= 0) {
        Milex.initiateFileDownload(route);
        return true;
    }
    ...
    if (mQuery(".form-exit-unlock-id").length) {
        if (mQuery(el).attr('data-ignore-formexit') != 'true') {
            ...
            Milex.unlockEntity(mQuery('.form-exit-unlock-model').val(), mQuery('.form-exit-unlock-id').val(), unlockParameter);
        }
    }
    ...
    MilexVars.routeInProgress = route;
    ...
    Milex.loadContent(route, link, method, target, showLoadingBar);
};
 ...
 * @param el
 */
Milex.activateChosenSelect = function(el, ignoreGlobal, jQueryVariant) {
    ...
    if (!noResultsText) {
        noResultsText = milexLang['chosenNoResults'];
    }
        ...
        if (mQuery(el).attr('data-new-route')) {
            ...
            mQuery(el).on('change', function () {
                ...
                if (mQuery(el).val() == 'new' && (mQuery(el).attr('data-popup') == "true" || mQuery(el).closest('.modal').length > 0)) {
                    ...
                    Milex.loadAjaxModalBySelectValue(this, 'new', url, mQuery(el).attr('data-header'));
                }
            });
        }

        var multiPlaceholder = milexLang['milex.core.lookup.search_options'],
            singlePlaceholder = milexLang['milex.core.lookup.search_options'];
    } else {
        var multiPlaceholder = milexLang['chosenChooseMore'],
            singlePlaceholder = milexLang['chosenChooseOne'];
    }
    ...
    if (isLookup) {
        ...
        if (searchTerm) {
            mQuery(el).ajaxChosen({
                type: 'GET',
                url: milexAjaxUrl + '?action=' + mQuery(el).attr('data-chosen-lookup'),
                dataType: 'json',
                afterTypeDelay: 2,
                minTermLength: 2,
 ...
 * @param el
 */
Milex.destroyChosen = function(el) {
    if(el.get(0)) {
 ...
 */
Milex.activateFieldTypeahead = function (field, target, options, action) {
    var fieldId = '#' + field;
    ...
    if (options && typeof options === 'String') {
        ...
        var fieldTypeahead = Milex.activateTypeahead(fieldId, {
            dataOptions: values,
            dataOptionKeys: keys,
            minLength: 0
        });
        ...
        var fieldTypeahead = Milex.activateTypeahead(fieldId, typeAheadOptions);
    }
    ...
    var callback = function (event, datum) {
        if (fieldEl.length && datum["value"]) {
            ...
            if (lookupCallback && typeof Milex[lookupCallback] == 'function') {
                Milex[lookupCallback](field, datum);
            }
        }
    };
 ...
 * @param el
 */
Milex.activateMultiSelect = function(el) {
 ...
 * @param container
 */
Milex.activateModalEmbeddedForms = function(container) {
    ...
    mQuery(container + " *[data-embedded-form='cancel']").on('click.embeddedform', function (event) {
        ...
        if (mQuery(this).attr('data-embedded-form-clear') === 'true') {
            Milex.resetForm(modal);
        }

        if (typeof mQuery(this).attr('data-embedded-form-callback') != 'undefined') {
            if (typeof window["Milex"][mQuery(this).attr('data-embedded-form-callback')] == 'function') {
                window["Milex"][mQuery(this).attr('data-embedded-form-callback')].apply('window', [this, modal]);
            }
        }
    });
 ...
 * @param container
 */
Milex.activateDateTimeInputs = function(el, type) {
 ...
 * @param modelName
 */
Milex.activateSearchAutocomplete = function (elId, modelName) {
    if (mQuery('#' + elId).length) {
        var livesearch = (mQuery('#' + elId).attr("data-toggle=['livesearch']")) ? true : false;

        var typeaheadObject = Milex.activateTypeahead('#' + elId, {
            prefetch: true,
            remote: false,
            limit: 0,
 ...
 * @param liveCacheVar
 */
Milex.activateLiveSearch = function (el, searchStrVar, liveCacheVar) {
    if (!mQuery(el).length) {
        return;
    }
    ...
    mQuery(el).on('focus', function () {
        Milex.currentSearchString = mQuery(this).val().trim();
    });
    mQuery(el).on('change keyup paste', {}, function (event) {
        ...
        if (!enterKeyPressed && Milex.currentSearchString && Milex.currentSearchString == searchStr) {
            return;
        }

        var target = mQuery(el).attr('data-target');
        var diff = searchStr.length - MilexVars[searchStrVar].length;
        ...
        //searchStr in MilexVars[liveCacheVar] ||
        if ((!searchStr && MilexVars[searchStrVar].length) || diff >= 3 || spaceKeyPressed || enterKeyPressed) {
            MilexVars[searchStrVar] = searchStr;
            event.data.livesearch = true;

            Milex.filterList(event,
                mQuery(el).attr('id'),
    ...
    if (mQuery(btn).length) {
        mQuery(btn).on('click', {'parent': mQuery(el).attr('id')}, function (event) {
            ...
            Milex.filterButtonClicked = true;
            Milex.filterList(event,
                event.data.parent,
 ...
 * @param el
 */
Milex.activateListFilterSelect = function(el) {
    ...
    if (isMultiple && prefixExceptions) {
        if (typeof Milex.listFilterValues == 'undefined') {
            Milex.listFilterValues = {};
        }

        // Store values for comparison on change
        Milex.listFilterValues[filterName] = mQuery(el).val();
    }
    ...
    mQuery(el).on('change', function() {
        ...
        if (prefixExceptions) {
            ...
            if (filterVal.length > 1) {
                for (var i=0; i<filterVal.length; i++) {
                    if (mQuery.inArray(filterVal[i], Milex.listFilterValues[filterName]) == -1) {
            ...
            Milex.listFilterValues[filterName] = filterVal;
        }
        ...
        Milex.loadContent(request, '', 'POST', mQuery(this).data('target'));
    });
};
 ...
 * @param el
 */
Milex.activateColorPicker = function(el, options) {
 ...
 * @returns {*}
 */
Milex.activateTypeahead = function (el, options) {
    ...
    if (options.prefetch || options.remote) {
        ...
        if (options.remote) {
            sourceOptions.remote = {
                url: milexAjaxUrl + "?action=" + options.action + "&filter=%QUERY",
        ...
        if (options.prefetch) {
            sourceOptions.prefetch = {
                url: milexAjaxUrl + "?action=" + options.action,
                filter: filterClosure
            };
        }
 ...
 * @param el
 */
Milex.activateSortable = function(el) {
 ...
 */
Milex.closeGlobalSearchResults = function () {
 ...
 * @param link
 */
Milex.initiateFileDownload = function (link) {
