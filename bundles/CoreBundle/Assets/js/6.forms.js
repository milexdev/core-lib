 ...
 * @param newNamePrefix
 */
Milex.renameFormElements = function(container, oldIdPrefix, oldNamePrefix, newIdPrefix, newNamePrefix) {
 ...
 * @param form
 */
Milex.ajaxifyForm = function (formName) {
    Milex.initializeFormFieldVisibilitySwitcher(formName);
    ...
    mQuery(form + ' input, ' + form + ' select').on('keydown.ajaxform', function (e) {
        if(e.keyCode == 13 && (e.metaKey || e.ctrlKey)) {
            if (MilexVars.formSubmitInProgress) {
                return false;
            }
    ...
    mQuery(form).on('submit.ajaxform', (function (e) {
        ...
        if (MilexVars.formSubmitInProgress) {
            ...
            if (callbackAsync && typeof Milex[callbackAsync] == 'function') {
                Milex[callbackAsync].apply(this, [form, function() {
                    Milex.postMilexForm(form);
                }]);
            } else {
                var callback = form.data('submit-callback');

                // Allow a callback to do stuff before submit and abort if needed
                if (callback && typeof Milex[callback] == 'function') {
                    if (!Milex[callback]()) {
                        return false;
                    }
                }
 ...
 * @param form
 */
Milex.postMilexForm = function(form) {
    MilexVars.formSubmitInProgress = true;
    Milex.postForm(form, function (response) {
        if (response.inMain) {
            Milex.processPageContent(response);
        } else {
            Milex.processModalContent(response, '#' + response.modalId);
        }
    });
};
 ...
 * @param callback
 */
Milex.postForm = function (form, callback) {
    ...
    form.ajaxSubmit({
        ...
        success: function (data) {
            ...
            if (data.redirect) {
                Milex.redirectWithBackdrop(data.redirect);
            } else {
                MilexVars.formSubmitInProgress = false;
                if (!inMain) {
                ...
                if (data.sessionExpired) {
                    ...
                    if (typeof callback == 'function') {
                        callback(data);
                    } else if (typeof Milex[callback] == 'function') {
                        Milex[callback](data);
                    }
                }
            }
        ...
        error: function (request, textStatus, errorThrown) {
            ...
            Milex.processAjaxError(request, textStatus, errorThrown, inMain);
        }
    });
};
 ...
 * @param formName
 */
Milex.initializeFormFieldVisibilitySwitcher = function (formName)
{
    Milex.switchFormFieldVisibilty(formName);
 ...
 */
Milex.switchFormFieldVisibilty = function (formName) {
 ...
 * @param response
 */
Milex.updateEntitySelect = function (response) {
    ...
    if (response.id) {
        ...
        if (mQueryParent(el).prop('disabled')) {
            ...
            var emptyOption = mQuery('<option value="">' + milexLang.chosenChooseOne + '</option>');
        } else {
    ...
    if (window.opener) {
        ...
        mQueryParent('#MilexSharedModal').modal('hide');
    }
};
 ...
 * @param changedId
 */
Milex.toggleYesNoButtonClass = function (changedId) {
    changedId = '#' + changedId;
 ...
 * @param el
 */
Milex.removeFormListOption = function (el) {
 ...
 * @param label
 */
Milex.createOption = function (value, label) {
 ...
 * @param valueOnChange
 */
Milex.updateFieldOperatorValue = function(field, action, valueOnChange, valueOnChangeArguments) {
    var fieldId = mQuery(field).attr('id');
    Milex.activateLabelLoadingIndicator(fieldId);
    ...
    Milex.ajaxActionRequest(action, {'alias': fieldAlias, 'operator': fieldOperator, 'changed': fieldType}, function(response) {
        if (typeof response.options != 'undefined') {
            ...
            if (mQuery('#'+fieldPrefix+'value_chosen').length) {
                ...
                Milex.destroyChosen(valueField);
            }
            ...
            if (!mQuery.isEmptyObject(response.options) && response.fieldType !== 'number') {
                ...
                mQuery.each(response.options, function(value, optgroup) {
                    if (typeof optgroup === 'object') {
                        ...
                        mQuery.each(optgroup, function(optVal, label) {
                            var option = Milex.createOption(optVal, label);
                        ...
                        var option = Milex.createOption(value, optgroup);
                ...
                Milex.activateChosenSelect(newValueField);
            } else {
                ...
                if (response.fieldType == 'date' || response.fieldType == 'datetime') {
                    Milex.activateDateTimeInputs(newValueField, response.fieldType);
                }
            }
            ...
            if (!mQuery.isEmptyObject(response.operators)) {
                ...
                Milex.destroyChosen(operatorField);
                ...
                var newOperatorField = mQuery('<select/>')
                    ...
                    .attr('onchange', 'Milex.updateLeadFieldValues(this)');
                mQuery.each(response.operators, function(optionVal, optionKey) {
                    newOperatorField.append(Milex.createOption(optionKey, optionVal));
                });
                newOperatorField.val(operatorField.val());
                operatorField.replaceWith(newOperatorField);
                Milex.activateChosenSelect(newOperatorField);
            }
        }
