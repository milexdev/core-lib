...
namespace Milex\CoreBundle\ErrorHandler {
    use Milex\CoreBundle\Exception\DatabaseConnectionException;
    use Milex\CoreBundle\Exception\ErrorHandlerException;
    ...
    class ErrorHandler
    {
        ...
        {
            $inline = $inTemplate;
            if (!$exception instanceof FatalThrowableError && defined('MILEX_DELEGATE_VIEW')) {
                $inline = true;
            }
            ...
            if (!empty($GLOBALS['MILEX_AJAX_DIRECT_RENDER'])) {
                $inline = true;
            }
        ...
        /**
         * Log fatal error to Milex's logs and throw exception for the parent generic error page to catch.
         *
         * @throws \Exception
         */
        ...
        private function generateResponse($error, $inline = true, $inTemplate = false)
        {
            ...
            if (!$inTemplate && !defined('MILEX_RENDERING_TEMPLATE') && $isAjax) {
                $dataArray = [];
            ...
            defined('MILEX_OFFLINE') or define('MILEX_OFFLINE', 1);
