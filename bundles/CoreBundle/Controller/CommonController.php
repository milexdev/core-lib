...
use Milex\CoreBundle\Helper\DataExporterHelper;
use Milex\CoreBundle\Helper\InputHelper;
use Milex\CoreBundle\Helper\TrailingSlashHelper;
use Milex\CoreBundle\Model\AbstractCommonModel;
 ...
 */
class CommonController extends Controller implements MilexController
{
    use FormThemeTrait;
    ...
    public function setFactory(MilexFactory $factory)
    {
    ...
    {
        return $this->container->get('milex.model.factory')->getModel($modelNameKey);
    }
    ...
    public function delegateView($args)
    {
        ...
        defined('MILEX_DELEGATE_VIEW') || define('MILEX_DELEGATE_VIEW', 1);

        if (!is_array($args)) {
            $args = [
                'contentTemplate' => $args,
                'passthroughVars' => [
                    'milexContent' => strtolower(InputHelper::alphanum($this->request->query->get('bundle'))),
                ],
            ];
        }
        ...
        if (!isset($args['viewParameters']['milexContent'])) {
            if (isset($args['passthroughVars']['milexContent'])) {
                $milexContent = $args['passthroughVars']['milexContent'];
            } else {
                $milexContent = strtolower(InputHelper::alphanum($this->request->query->get('bundle')));
            }
            $args['viewParameters']['milexContent'] = $milexContent;
        }
    ...
    public function removeTrailingSlashAction(Request $request)
    {
        ...
        $trailingSlashHelper = $this->get('milex.helper.trailing_slash');
    ...
    {
        return $this->redirect($this->generateUrl('milex_dashboard_index'), 301);
    }
    ...
    {
        $returnUrl = array_key_exists('returnUrl', $args) ? $args['returnUrl'] : $this->generateUrl('milex_dashboard_index');
    ...
    {
        defined('MILEX_AJAX_VIEW') || define('MILEX_AJAX_VIEW', 1);
        ...
        if ($contentTemplate) {
            if ($forward) {
                ...
                $GLOBALS['MILEX_AJAX_DIRECT_RENDER'] = 1; // for error handling
                $newContent                           = $this->renderView($contentTemplate, $parameters);

                unset($GLOBALS['MILEX_AJAX_DIRECT_RENDER']);
            }
        }
        ...
        $passthrough['browserNotifications'] = $this->get('session')->get('milex.browser.notifications', []);
        $this->get('session')->set('milex.browser.notifications', []);
    ...
    public function renderException(\Exception $e)
    {
        ...
        return $this->forward('MilexCoreBundle:Exception:show', $parameters, $query);
    }
     ...
     */
    public function accessDenied($batch = false, $msg = 'milex.core.url.error.401')
    {
        ...
        if ($batch) {
            return [
                'type' => 'error',
                'msg'  => $this->translator->trans('milex.core.error.accessdenied', [], 'flashes'),
            ];
        }
    }
     ...
     * @return Response
     */
    public function notFound($msg = 'milex.core.url.error.404')
    {
        ...
        if (!empty($page_404)) {
            ...
            if (!empty($page) && $page->getIsPublished() && !empty($page->getCustomHtml())) {
                ...
                return $this->redirectToRoute('milex_page_public', ['slug' => $slug]);
            }
        }
     ...
     * @return JsonResponse
     */
    public function modalAccessDenied($msg = 'milex.core.error.accessdenied')
    {
    ...
    {
        return $this->renderView('MilexCoreBundle:Notification:flash_messages.html.php');
    }
    ...
    protected function getNotificationContent(Request $request = null)
    {
        ...
        $afterId = $request->get('milexLastNotificationId', null);
        ...
        return [
            'content' => ($notifications || $updateMessage) ? $this->renderView('MilexCoreBundle:Notification:notification_messages.html.php', [
     ...
     *
     * @deprecated Will be removed in Milex 3.0 as unused.
     */
    public function addNotification($message, $type = null, $isRead = true, $header = null, $iconClass = null, \DateTime $datetime = null)
    {
        /** @var \Milex\CoreBundle\Model\NotificationModel $notificationModel */
    ...
    public function addBrowserNotification($message, $messageVars = [], $domain = 'flashes', $title = null, $icon = null, $addNotification = true, $type = 'notice')
    {
        ...
        $session                = $this->get('session');
        $browserNotifications   = $session->get('milex.browser.notifications', []);
        ...
        $session->set('milex.browser.notifications', $browserNotifications);

        if (!defined('MILEX_INSTALLER') && $addNotification) {
            switch ($type) {
                case 'warning':
            ...
            $lastActive = $this->request->get('milexUserLastActive', 0);
    ...
    {
        /** @var \Milex\CoreBundle\Helper\ExportHelper */
        $exportHelper = $this->get('milex.helper.export');

        if (!in_array($type, $exportHelper->getSupportedExportTypes())) {
            throw new BadRequestHttpException($this->translator->trans('milex.error.invalid.export.type', ['%type%' => $type]));
        }
