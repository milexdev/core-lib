...
abstract class AbstractStandardFormController extends AbstractFormController
{
    ...
    /**
     * @deprecated The PHP templating engine is deprecated and will be removed in Milex 5. Please use ENGINE_TWIG instead.
     */
    ...
    {
        $page      = $this->get('session')->get('milex.'.$this->getSessionBase().'.page', 1);
    ...
    protected function checkActionPermission($action, $entity = null, $objectId = null)
    {
        ...
        if ($entity) {
            switch ($action) {
                ...
                case 'delete':
                case 'batchDelete':
                    return $this->get('milex.security')->hasEntityAccess(
    ...
    {
        return 'milex_'.str_replace('milex_', '', $this->getRouteBase().'_action');
    }

    /**
     * Get controller base if different than MilexCoreBundle:Standard.
     *
     * @return string
     */
    protected function getControllerBase()
    {
        return 'MilexCoreBundle:Standard';
    }
        ...
        switch ($action) {
            ...
            case 'edit':
                ...
                if (is_object($objectId)) {
                    $entity   = $objectId;
                    $isClone  = true;
                    $objectId = (!empty($this->sessionId)) ? $this->sessionId : 'milex_'.sha1(uniqid(mt_rand(), true));
                } elseif (false !== strpos($objectId, 'milex_')) {
                    $isClone = true;
                    $entity  = $model->getEntity();
                } else {
                    $entity = $model->getEntity($objectId);
    ...
    {
        return 'milex_'.str_replace('milex_', '', $this->getRouteBase().'_index');
    }
    ...
    /**
     * Get the route base for getIndexRoute() and getActionRoute() if they do not meet the milex_*_index and milex_*_action standards.
     *
     * @return mixed
     */
    ...
    /**
     * Get template base different than MilexCoreBundle:Standard.
     *
     * @return string
     */
    ...
    protected function getTemplateName($file, string $engine = self::ENGINE_TWIG)
    {
        ...
        } elseif ($this->get('templating')->exists('MilexCoreBundle:Standard:'.$file)) {
            return 'MilexCoreBundle:Standard:'.$file;
    ...
    {
        return 'milex.'.$this->getModelName();
    }
    ...
    protected function indexStandard($page = null)
    {
        ...
        if (empty($page)) {
            $page = $session->get('milex.'.$this->getSessionBase().'.page', 1);
        }

        //set limits
        $limit = $session->get('milex.'.$this->getSessionBase().'.limit', $this->coreParametersHelper->get('default_pagelimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('milex.'.$this->getSessionBase().'.filter', ''));
        $session->set('milex.'.$this->getSessionBase().'.filter', $search);
        ...
        $orderBy    = $session->get('milex.'.$this->getSessionBase().'.orderby', $repo->getTableAlias().'.'.$this->getDefaultOrderColumn());
        $orderByDir = $session->get('milex.'.$this->getSessionBase().'.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('milex.'.$this->getSessionBase().'.page', $lastPage);
        ...
        $session->set('milex.'.$this->getSessionBase().'.page', $page);
