...
use Milex\ReportBundle\Builder\MilexReportBuilder;
...
use Milex\ReportBundle\Event\ReportQueryEvent;
use Milex\ReportBundle\Generator\ReportGenerator;
...
class ReportModel extends FormModel
{
     ...
     * {@inheritdoc}
     *
     * @return \Milex\ReportBundle\Entity\ReportRepository
     */
    ...
    public function getGraphList($context = 'all')
    {
        ...
        foreach ($graphData as $key => $details) {
            $return->choices[$key] = $this->translator->trans($key).' ('.$this->translator->trans('milex.report.graph.'.$details['type']).')';
        }
        ...
        switch ($format) {
            ...
            case 'html':
                $content = $this->templatingHelper->getTemplating()->renderResponse(
                    'MilexReportBundle:Report:export.html.php',
                    [
    ...
    public function getReportData(Report $entity, FormFactoryInterface $formFactory = null, array $options = [])
    {
        ...
        $orderBy    = $this->session->get('milex.report.'.$entity->getId().'.orderby', '');
        $orderByDir = $this->session->get('milex.report.'.$entity->getId().'.orderbydir', 'ASC');
        ...
        $this->session->set('milex.report.'.$entity->getId().'.page', $reportPage);
        ...
        if (empty($options['ignoreTableData']) && !empty($selectedColumns)) {
            if ($paginate) {
                ...
                $limit = $this->session->get('milex.report.'.$entity->getId().'.limit', $this->defaultPageLimit);
    ...
    private function getOperatorOptions(array $data)
    {
        if (isset($data['operators'])) {
            ...
            if (!array_key_exists($operator, MilexReportBuilder::OPERATORS)) {
                $operator = 'default';
            }

            $options = MilexReportBuilder::OPERATORS[$operator];
        }
    ...
    {
        return MILEX_ENV == 'dev' || $this->coreParametersHelper->get('debug');
    }
}
